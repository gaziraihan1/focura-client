...
import { RefundSection } from '@/components/public/refund/RefundSection'

describe('RefundSection', () => {
  it('renders section title and content', () => {
    render(
      <RefundSection id="test" title="Refund Test" icon={({ ...p }: Record<string, unknown>) => <svg {...p} />} index={1}>
        <p>Content</p>
      </RefundSection>
    )
    expect(screen.getByText('Refund Test')).toBeInTheDocument()
