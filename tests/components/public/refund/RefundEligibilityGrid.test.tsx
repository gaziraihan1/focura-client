...
import { RefundEligibilityGrid } from '@/components/public/refund/RefundEligibilityGrid'

describe('RefundEligibilityGrid', () => {
  it('renders eligible header', () => {
    render(<RefundEligibilityGrid />)
    expect(screen.getByText('Eligible for Refund')).toBeInTheDocument()
  })

  it('renders not eligible header', () => {
    render(<RefundEligibilityGrid />)
    expect(screen.getByText('Not Eligible for Refund')).toBeInTheDocument()
  })

  it('renders eligible items', () => {
    render(<RefundEligibilityGrid />)
    expect(screen.getByText('First-time subscription purchase (any paid plan)')).toBeInTheDocument()
    expect(screen.getByText(/Proven technical issue/)).toBeInTheDocument()
  })

  it('renders not eligible items', () => {
    render(<RefundEligibilityGrid />)
    expect(screen.getByText('Subscription renewals (monthly or annual)')).toBeInTheDocument()
    expect(screen.getByText(/many tasks, workspaces, or sessions created/)).toBeInTheDocument()
  })
})
