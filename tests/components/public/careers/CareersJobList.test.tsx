import { describe, it, expect, vi } from "vitest";
import { render, screen } from "@testing-library/react";

vi.mock("lucide-react", () => ({
  Loader2: (props: React.SVGProps<SVGSVGElement>) => <svg data-testid="icon-Loader2" {...props} />,
}));

vi.mock("@/lib/axios", () => ({
  api: { get: vi.fn().mockResolvedValue({ success: true, data: [] }) },
  unwrap: (res: { data?: unknown }) => res?.data ?? res,
  unwrapList: (res: { data?: unknown }) => (Array.isArray(res?.data) ? res.data : []),
}));

vi.mock("@/components/public/careers/CareersFilters", () => ({
  CareersFilters: () => <div data-testid="careers-filters" />,
}));

vi.mock("@/components/public/careers/CareersJobCard", () => ({
  CareersJobCard: () => <div data-testid="careers-job-card" />,
}));

vi.mock("@/components/public/careers/CareersEmpty", () => ({
  CareersEmpty: () => <div data-testid="careers-empty" />,
}));

vi.mock("@/components/public/careers/CareersApplyModal", () => ({
  CareersApplyModal: () => <div data-testid="careers-apply-modal" />,
}));

vi.mock("@/components/public/careers/CareersJobDetailModal", () => ({
  CareersJobDetailModal: () => <div data-testid="careers-detail-modal" />,
}));

import CareersJobList from "@/components/public/careers/CareersJobList";

describe("CareersJobList", () => {
  it("renders loading state initially", () => {
    render(<CareersJobList />);
    expect(screen.getByTestId("icon-Loader2")).toBeInTheDocument();
  });

  it("renders the CareersEmpty component when no jobs", async () => {
    render(<CareersJobList />);
    // After fetch resolves with empty jobs, CareersEmpty should appear
    const { waitFor } = await import("@testing-library/react");
    await waitFor(() => {
      expect(screen.getByTestId("careers-empty")).toBeInTheDocument();
    });
  });
});
