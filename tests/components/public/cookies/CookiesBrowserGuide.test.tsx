...
import { CookiesBrowserGuide } from '@/components/public/cookies/CookiesBrowserGuide'

describe('CookiesBrowserGuide', () => {
  it('renders all browser names', () => {
    render(<CookiesBrowserGuide />)
  ...
  it('renders shortcuts', () => {
    render(<CookiesBrowserGuide />)
    expect(screen.getByText('chrome://settings/cookies')).toBeInTheDocument()
    expect(screen.getByText('about:preferences#privacy')).toBeInTheDocument()
  })

  it('renders "Official guide" links', () => {
    render(<CookiesBrowserGuide />)
    const guideLinks = screen.getAllByText('Official guide')
    expect(guideLinks.length).toBe(4)
  })
})
