...
import TemplatesHowItWorks from '@/components/Templates/TemplateshowItWorks'

describe('TemplatesHowItWorks', () => {
  it('renders the heading', () => {
    render(<TemplatesHowItWorks />)
    expect(screen.getByText('How it works')).toBeInTheDocument()
    expect(screen.getByText('From template to productive in minutes.')).toBeInTheDocument()
  })

  it('renders all 3 steps', () => {
    render(<TemplatesHowItWorks />)
    expect(screen.getByText('Browse & choose')).toBeInTheDocument()
    expect(screen.getByText('Import in one click')).toBeInTheDocument()
    expect(screen.getByText('Customise & ship')).toBeInTheDocument()
  })
  ...
  it('renders "What gets cloned" section', () => {
    render(<TemplatesHowItWorks />)
    expect(screen.getByText('What gets cloned when you import a template')).toBeInTheDocument()
    expect(screen.getByText('Tasks & subtasks')).toBeInTheDocument()
    expect(screen.getByText('Labels & colours')).toBeInTheDocument()
  })

  it('renders the Getting Started guide link', () => {
    render(<TemplatesHowItWorks />)
    expect(screen.getByText('Read the full Getting Started guide')).toBeInTheDocument()
  })
})
