...
describe("AdminJobTable", () => {
  it("renders empty state when no jobs", async () => {
    const AdminJobTable = (await import("@/components/admin-dashboard/careers/AdminJobTable")).default;
    render(
      <AdminJobTable
        jobs={[]}
        onEdit={vi.fn()}
    ...
    expect(screen.getByText("No job postings yet. Create your first one.")).toBeInTheDocument();
  });
