...
describe("MessageModal", () => {
  it("renders message modal with details", async () => {
    const { MessageModal } = await import("@/components/admin-dashboard/contact/MessageModal");
    const onClose = vi.fn();
    render(
      <MessageModal
        message={{
    ...
    expect(screen.getAllByText("Support request").length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText("Bob")).toBeInTheDocument();
    expect(screen.getByText("b@test.com")).toBeInTheDocument();
    expect(screen.getByText("Message: Need help")).toBeInTheDocument();
  });
  ...
  it("calls onClose when close button clicked", async () => {
    ...
    // The close button contains the X icon SVG
    const closeBtn = document.querySelector("button.shrink-0.p-1\\.5");
