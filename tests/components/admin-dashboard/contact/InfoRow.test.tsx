...
describe("InfoRow", () => {
  it("renders label and value", async () => {
    const { InfoRow } = await import("@/components/admin-dashboard/contact/InfoRow");
    render(<InfoRow label="Name" value="John" />);
    expect(screen.getByText("Name")).toBeInTheDocument();
    expect(screen.getByText("John")).toBeInTheDocument();
  });

  it("renders email variant with mailto link", async () => {
    const { InfoRow } = await import("@/components/admin-dashboard/contact/InfoRow");
    render(<InfoRow label="Email" value="j@test.com" isEmail />);
    const link = screen.getByText("j@test.com");
    expect(link.closest("a")).toHaveAttribute("href", "mailto:j@test.com");
  });

  it("renders mono variant", async () => {
    const { InfoRow } = await import("@/components/admin-dashboard/contact/InfoRow");
    render(<InfoRow label="ID" value="abc123" mono />);
    expect(screen.getByText("abc123").className).toContain("font-mono");
  });

  it("renders with icon", async () => {
    const { InfoRow } = await import("@/components/admin-dashboard/contact/InfoRow");
    render(<InfoRow icon={<span data-testid="test-icon" />} label="Test" value="val" />);
