...
describe("AdminTable", () => {
  it("renders loading skeleton", async () => {
    const { AdminTable } = await import("@/components/admin-dashboard/AdminTable");
    const { container } = render(
      <AdminTable
        columns={[{ key: "a", header: "Col A", render: (r: Record<string, unknown>) => r.a }]}
        data={[]}
        isLoading
        skeletonRows={3}
      />
  ...
  it("renders data rows", async () => {
    ...
    render(
      <AdminTable
        columns={[{ key: "name", header: "Name", render: (r: Record<string, unknown>) => r.name }]}
