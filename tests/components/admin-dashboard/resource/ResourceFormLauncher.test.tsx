...
describe("ResourceFormLauncher", () => {
  it("renders tabs and creates panel", async () => {
    const { ResourceFormLauncher } = await import("@/components/admin-dashboard/resource/ResourceFormLauncher");
    render(<ResourceFormLauncher />);
    expect(screen.getAllByText("Popular resources").length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText("Product updates")).toBeInTheDocument();
    expect(screen.getByText("New resource")).toBeInTheDocument();
  });

  it("toggles to product updates tab", async () => {
    const { ResourceFormLauncher } = await import("@/components/admin-dashboard/resource/ResourceFormLauncher");
    render(<ResourceFormLauncher />);
