...
describe("ResourcePagination", () => {
  it("renders nothing when totalPages <= 1", async () => {
    const { ResourcePagination } = await import("@/components/admin-dashboard/resource/ResourcePagination");
    const { container } = render(
      <ResourcePagination page={1} totalPages={1} onPageChange={vi.fn()} />
    );
  ...
  it("renders pagination controls", async () => {
    const { ResourcePagination } = await import("@/components/admin-dashboard/resource/ResourcePagination");
    render(
      <ResourcePagination page={2} totalPages={5} onPageChange={vi.fn()} />
    );
    expect(screen.getByText("Page 2 of 5")).toBeInTheDocument();
  });

  it("calls onPageChange on next/prev click", async () => {
    const onPageChange = vi.fn();
    const { ResourcePagination } = await import("@/components/admin-dashboard/resource/ResourcePagination");
    render(
      <ResourcePagination page={2} totalPages={5} onPageChange={onPageChange} />
    );
