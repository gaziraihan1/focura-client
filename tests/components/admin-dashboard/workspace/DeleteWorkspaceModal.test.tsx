...
describe("DeleteWorkspaceModal", () => {
  it("renders nothing when not open", async () => {
    const { DeleteWorkspaceModal } = await import("@/components/admin-dashboard/workspace/DeleteWorkspaceModal");
    const { container } = render(
      <DeleteWorkspaceModal workspaceSlug="ws" workspaceName="Test WS" isOpen={false} onClose={vi.fn()} />
    );
    expect(container.innerHTML).toBe("");
  });

  it("renders modal with workspace name", async () => {
    const { DeleteWorkspaceModal } = await import("@/components/admin-dashboard/workspace/DeleteWorkspaceModal");
    render(
  ...
  it("toggles hard delete mode", async () => {
    const { DeleteWorkspaceModal } = await import("@/components/admin-dashboard/workspace/DeleteWorkspaceModal");
    render(
      <DeleteWorkspaceModal workspaceSlug="ws" workspaceName="Test WS" isOpen onClose={vi.fn()} />
    );
    // Default mode is suspend — "Suspend" appears in the type selector and the action button
    expect(screen.getAllByText("Suspend").length).toBeGreaterThanOrEqual(1);
    // Click Permanent to switch to hard delete
    fireEvent.click(screen.getByText("Permanent").closest("button")!);
    expect(screen.getByText("Delete Forever")).toBeInTheDocument();
    expect(screen.getByText(/This will permanently delete all projects/)).toBeInTheDocument();
  });

  it("shows validation when name doesn't match", async () => {
    const { DeleteWorkspaceModal } = await import("@/components/admin-dashboard/workspace/DeleteWorkspaceModal");
    render(
      <DeleteWorkspaceModal workspaceSlug="ws" workspaceName="Test WS" isOpen onClose={vi.fn()} />
    );
