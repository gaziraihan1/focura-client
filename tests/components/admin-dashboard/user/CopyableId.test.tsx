...
describe("CopyableId", () => {
  it("renders id and copies on click", async () => {
    const { CopyableId } = await import("@/components/admin-dashboard/user/CopyableId");
    const writeText = vi.fn().mockResolvedValue(undefined);
    Object.assign(navigator, { clipboard: { writeText } });
    render(<CopyableId id="cm_user_abc123" />);
    expect(screen.getByText("cm_user_abc123")).toBeInTheDocument();
    fireEvent.click(screen.getByText("cm_user_abc123").closest("button")!);
    expect(writeText).toHaveBeenCalledWith("cm_user_abc123");
  });
});
