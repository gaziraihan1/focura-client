...
describe("BanUserModal", () => {
  it("renders nothing when not open", async () => {
    const { BanUserModal } = await import("@/components/admin-dashboard/user/BanUserModal");
    const { container } = render(
      <BanUserModal userId="1" userName="Test" isOpen={false} onClose={vi.fn()} />
    );
  ...
  it("renders modal when open", async () => {
    const { BanUserModal } = await import("@/components/admin-dashboard/user/BanUserModal");
    render(
      <BanUserModal userId="1" userName="Test User" isOpen onClose={vi.fn()} />
    );
    expect(screen.getAllByText("Ban User").length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText(/Test User/)).toBeInTheDocument();
  });

  it("disables ban button when reason is empty", async () => {
    const { BanUserModal } = await import("@/components/admin-dashboard/user/BanUserModal");
    render(
      <BanUserModal userId="1" userName="U" isOpen onClose={vi.fn()} />
    );
    const btn = screen.getByText("Ban User", { selector: "button" });
