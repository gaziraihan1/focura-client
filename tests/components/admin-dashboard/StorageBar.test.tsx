...
describe("StorageBar", () => {
  it("renders usage bar with label", async () => {
    const { StorageBar } = await import("@/components/admin-dashboard/StorageBar");
  ...
  it("hides label when showLabel is false", async () => {
    const { StorageBar } = await import("@/components/admin-dashboard/StorageBar");
    const { container } = render(<StorageBar usedMb={100} maxMb={1000} showLabel={false} />);
    expect(container.querySelector("p")).toBeNull();
  });

  it("renders destructive color at 90%+", async () => {
    const { StorageBar } = await import("@/components/admin-dashboard/StorageBar");
