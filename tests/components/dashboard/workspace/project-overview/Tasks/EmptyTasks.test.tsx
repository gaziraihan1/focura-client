...
describe('Workspaces/project/Tasks – EmptyTasks', () => {
  it('renders no tasks message', async () => {
    const { EmptyTasks } = await import('@/components/dashboard/workspace/project-overview/Tasks/EmptyTasks')
    render(<EmptyTasks onAddTask={vi.fn()} canCreate={true} isArchived={false} />)
    expect(screen.getByText('No tasks found')).toBeInTheDocument()
  })
  it('shows create button when canCreate and not archived', async () => {
    const { EmptyTasks } = await import('@/components/dashboard/workspace/project-overview/Tasks/EmptyTasks')
    render(<EmptyTasks onAddTask={vi.fn()} canCreate={true} isArchived={false} />)
    expect(screen.getByText('Create task')).toBeInTheDocument()
  })
  it('hides create button when archived', async () => {
    const { EmptyTasks } = await import('@/components/dashboard/workspace/project-overview/Tasks/EmptyTasks')
    render(<EmptyTasks onAddTask={vi.fn()} canCreate={true} isArchived={true} />)
    expect(screen.queryByText('Create task')).not.toBeInTheDocument()
  })
  it('hides create button when cannot create', async () => {
    const { EmptyTasks } = await import('@/components/dashboard/workspace/project-overview/Tasks/EmptyTasks')
    render(<EmptyTasks onAddTask={vi.fn()} canCreate={false} isArchived={false} />)
    expect(screen.queryByText('Create task')).not.toBeInTheDocument()
  })
  it('calls onAddTask', async () => {
    const onAddTask = vi.fn()
    const { EmptyTasks } = await import('@/components/dashboard/workspace/project-overview/Tasks/EmptyTasks')
    render(<EmptyTasks onAddTask={onAddTask} canCreate={true} isArchived={false} />)
