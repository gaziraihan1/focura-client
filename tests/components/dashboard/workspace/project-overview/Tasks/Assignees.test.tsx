...
describe('Workspaces/project/Tasks – Assignees', () => {
  it('renders empty state when no assignees', async () => {
    const { Assignees } = await import('@/components/dashboard/workspace/project-overview/Tasks/Assignees')
    const { container } = render(<Assignees assignees={[]} />)
    expect(container.querySelector('.bg-muted')).toBeInTheDocument()
  })
  it('renders avatar with initials when no image', async () => {
    const { Assignees } = await import('@/components/dashboard/workspace/project-overview/Tasks/Assignees')
    render(<Assignees assignees={[{ user: { id: 'u1', name: 'Alice', image: null } }]} />)
    expect(screen.getByText('A')).toBeInTheDocument()
  })
  it('renders image when available', async () => {
    const { Assignees } = await import('@/components/dashboard/workspace/project-overview/Tasks/Assignees')
    render(<Assignees assignees={[{ user: { id: 'u1', name: 'Alice', image: '/img.png' } }]} />)
    expect(screen.getByRole('img')).toBeInTheDocument()
  })
  it('shows +count for >3 assignees', async () => {
    const { Assignees } = await import('@/components/dashboard/workspace/project-overview/Tasks/Assignees')
    render(
      <Assignees assignees={[
        { user: { id: 'u1', name: 'A', image: null } },
        { user: { id: 'u2', name: 'B', image: null } },
        { user: { id: 'u3', name: 'C', image: null } },
        { user: { id: 'u4', name: 'D', image: null } },
      ]} />
    )
