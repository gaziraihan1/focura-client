...
describe('Workspaces/project/Settings – DangerTab', () => {
  it('renders archive section when not archived', async () => {
    const { DangerTab } = await import('@/components/dashboard/workspace/project-overview/Settings/DangerTab')
    render(<DangerTab project={{ id: 'p1', name: 'P', status: 'ACTIVE' } as any} canManage={true} isOwnerOrAdmin={false} workspaceSlug="ws" />)
    expect(screen.getAllByText('Archive Project').length).toBeGreaterThanOrEqual(1)
  })
  it('renders delete section for owner', async () => {
    const { DangerTab } = await import('@/components/dashboard/workspace/project-overview/Settings/DangerTab')
    render(<DangerTab project={{ id: 'p1', name: 'P', status: 'ACTIVE' } as any} canManage={true} isOwnerOrAdmin={true} workspaceSlug="ws" />)
    expect(screen.getByText('Delete Project')).toBeInTheDocument()
  })
  it('shows locked message when no permissions', async () => {
    const { DangerTab } = await import('@/components/dashboard/workspace/project-overview/Settings/DangerTab')
    render(<DangerTab project={{ id: 'p1', name: 'P', status: 'ACTIVE' } as any} canManage={false} isOwnerOrAdmin={false} workspaceSlug="ws" />)
    expect(screen.getByText(/don't have permission/)).toBeInTheDocument()
  })
  it('renders unarchive when archived', async () => {
    const { DangerTab } = await import('@/components/dashboard/workspace/project-overview/Settings/DangerTab')
    render(<DangerTab project={{ id: 'p1', name: 'P', status: 'ARCHIVED' } as any} canManage={true} isOwnerOrAdmin={false} workspaceSlug="ws" />)
    expect(screen.getByText('Unarchive Project')).toBeInTheDocument()
  })
  it('renders change status section', async () => {
    const { DangerTab } = await import('@/components/dashboard/workspace/project-overview/Settings/DangerTab')
    render(<DangerTab project={{ id: 'p1', name: 'P', status: 'ACTIVE' } as any} canManage={true} isOwnerOrAdmin={false} workspaceSlug="ws" />)
