...
describe("Pagination (Announcements)", () => {
  it("renders nothing when totalPages <= 1", async () => {
    const { Pagination } = await import("@/components/dashboard/workspace/project-overview/Announcements/Pagination");
    const { container } = render(
      <Pagination
        pagination={{ page: 1, pageSize: 10, totalCount: 5, totalPages: 1, hasNext: false, hasPrev: false }}
        currentPage={1}
        onPageChange={vi.fn()}
      />
    );
  ...
  it("renders page controls", async () => {
    ...
    render(
      <Pagination
        pagination={{ page: 1, pageSize: 10, totalCount: 30, totalPages: 3, hasNext: true, hasPrev: false }}
        currentPage={1}
        onPageChange={vi.fn()}
      />
    );
