...
describe("ProjectDeadlineRiskPanel", () => {
  it("renders risk panel with high priority tasks", async () => {
    const { ProjectDeadlineRiskPanel } = await import("@/components/dashboard/workspace/project-overview/Analytics/ProjectDeadlineRiskPanel");
    render(
      <ProjectDeadlineRiskPanel
        data={{
          riskLevel: "medium",
          dueIn3DaysCount: 3,
          dueIn7DaysCount: 5,
  ...
  it("renders no urgent deadlines when all clear", async () => {
    const { ProjectDeadlineRiskPanel } = await import("@/components/dashboard/workspace/project-overview/Analytics/ProjectDeadlineRiskPanel");
    render(
      <ProjectDeadlineRiskPanel
        data={{
    ...
    expect(screen.getByText("No urgent deadlines - you're on track!")).toBeInTheDocument();
  });
});
