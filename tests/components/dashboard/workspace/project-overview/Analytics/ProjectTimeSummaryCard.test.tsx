...
describe("ProjectTimeSummaryCard", () => {
  it("renders time summary with data", async () => {
    const { ProjectTimeSummaryCard } = await import("@/components/dashboard/workspace/project-overview/Analytics/ProjectTimeSummaryCard");
    render(
      <ProjectTimeSummaryCard
        data={{
          totalHours: 150.5,
          avgHoursPerMember: 37.6,
          topContributors: [{ userId: "1", userName: "Alice", userEmail: "a@b.com", hours: 50 }],
        }}
        days={30}
      />
    );
    expect(screen.getByText("Time Summary")).toBeInTheDocument();
    expect(screen.getByText("150.5h")).toBeInTheDocument();
    expect(screen.getByText("Alice")).toBeInTheDocument();
  });

  it("renders null data state", async () => {
    const { ProjectTimeSummaryCard } = await import("@/components/dashboard/workspace/project-overview/Analytics/ProjectTimeSummaryCard");
    render(<ProjectTimeSummaryCard data={null} days={30} />);
    expect(screen.getByText("No time tracking data available")).toBeInTheDocument();
  });
});
