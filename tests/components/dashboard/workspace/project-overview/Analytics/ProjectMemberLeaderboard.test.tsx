...
describe("ProjectMemberLeaderboard", () => {
  it("renders leaderboard with data", async () => {
    const { ProjectMemberLeaderboard } = await import("@/components/dashboard/workspace/project-overview/Analytics/ProjectMemberLeaderboard");
    render(
      <ProjectMemberLeaderboard
        data={[
          { userId: "1", userName: "Alice", userEmail: "a@b.com", role: "ADMIN", completedTasks: 10, totalHours: 50, commentsCount: 20, contributionScore: 150 },
          { userId: "2", userName: "Bob", userEmail: "b@c.com", role: "MEMBER", completedTasks: 5, totalHours: 30, commentsCount: 10, contributionScore: 80 },
        ]}
      />
    );
  ...
  it("renders nothing when empty", async () => {
    const { ProjectMemberLeaderboard } = await import("@/components/dashboard/workspace/project-overview/Analytics/ProjectMemberLeaderboard");
    const { container } = render(<ProjectMemberLeaderboard data={[]} />);
