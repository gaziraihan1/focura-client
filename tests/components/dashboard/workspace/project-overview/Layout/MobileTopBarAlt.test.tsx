...
describe('Workspaces/project/Layout – MobileTopBar', () => {
  it('renders project name', async () => {
    const { MobileTopBar } = await import('@/components/dashboard/workspace/project-overview/Layout/MobileTopBar')
    render(<MobileTopBar projectName="My Project" projectColor="#3b82f6" onOpen={vi.fn()} />)
    expect(screen.getByText('My Project')).toBeInTheDocument()
  })
  it('renders current label', async () => {
    const { MobileTopBar } = await import('@/components/dashboard/workspace/project-overview/Layout/MobileTopBar')
    render(<MobileTopBar projectName="P" projectColor="#3b82f6" currentLabel="Tasks" onOpen={vi.fn()} />)
    expect(screen.getByText('Tasks')).toBeInTheDocument()
  })
  it('calls onOpen', async () => {
    const onOpen = vi.fn()
    const { MobileTopBar } = await import('@/components/dashboard/workspace/project-overview/Layout/MobileTopBar')
    render(<MobileTopBar projectName="P" projectColor="#3b82f6" onOpen={onOpen} />)
    fireEvent.click(screen.getByRole('button'))
    expect(onOpen).toHaveBeenCalled()
  })
  it('shows default Project text when no name', async () => {
    const { MobileTopBar } = await import('@/components/dashboard/workspace/project-overview/Layout/MobileTopBar')
    render(<MobileTopBar projectName={undefined} projectColor="#3b82f6" onOpen={vi.fn()} />)
    expect(screen.getByText('Project')).toBeInTheDocument()
  })
  it('hides label when not provided', async () => {
    const { MobileTopBar } = await import('@/components/dashboard/workspace/project-overview/Layout/MobileTopBar')
    render(<MobileTopBar projectName="P" projectColor="#3b82f6" onOpen={vi.fn()} />)
