...
describe("WorkspaceSidebar", () => {
  const baseNav = [
    { name: "Overview", href: "/ws", icon: vi.fn(() => null), match: (p: string) => p === "/ws" },
    { name: "Projects", href: "/ws/projects", icon: vi.fn(() => null), match: (p: string) => p.startsWith("/ws/projects") },
  ];
  const ws = { id: "1", name: "My WS", slug: "my-ws", color: "#3b82f6" } as any;

  it("renders workspace name and navigation", async () => {
    const { WorkspaceSidebar } = await import("@/components/dashboard/workspace/layout/WorkspaceSidebar");
    render(
      <WorkspaceSidebar
        workspace={ws}
        navigation={baseNav}
  ...
  it("renders loading skeletons", async () => {
    ...
    expect(screen.queryByText("My WS")).not.toBeInTheDocument();
  });

  it("renders locked items with upgrade button", async () => {
    const { WorkspaceSidebar } = await import("@/components/dashboard/workspace/layout/WorkspaceSidebar");
    const lockedNav = [...baseNav, { name: "Analytics", icon: vi.fn(() => null), match: () => false, locked: true }];
    render(
      <WorkspaceSidebar
        workspace={ws}
        navigation={lockedNav}
        pathname="/ws"
        slug="my-ws"
        sidebarOpen={false}
        onSidebarClose={vi.fn()}
  ...
  it("renders settings and all workspaces links", async () => {
    const { WorkspaceSidebar } = await import("@/components/dashboard/workspace/layout/WorkspaceSidebar");
    render(
      <WorkspaceSidebar
        workspace={ws}
  ...
  it("shows mobile overlay when sidebarOpen", async () => {
    const { WorkspaceSidebar } = await import("@/components/dashboard/workspace/layout/WorkspaceSidebar");
    render(
      <WorkspaceSidebar
        workspace={ws}
