...
describe('WorkspaceNewTask/IntentSelector', () => {
  it('renders intent options', async () => {
    const { IntentSelector } = await import('@/components/dashboard/tasks/workspace-new-task/IntentSelector')
    render(<IntentSelector selectedIntent="EXECUTION" onIntentChange={vi.fn()} />)
    expect(screen.getByText('Do Work')).toBeInTheDocument()
  })
  it('calls onIntentChange on click', async () => {
    const onIntentChange = vi.fn()
    const { IntentSelector } = await import('@/components/dashboard/tasks/workspace-new-task/IntentSelector')
    render(<IntentSelector selectedIntent="EXECUTION" onIntentChange={onIntentChange} />)
    fireEvent.click(screen.getByText('Think & Plan'))
    expect(onIntentChange).toHaveBeenCalledWith('PLANNING')
  })
})
