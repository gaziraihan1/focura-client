...
describe("TeamTaskFiltersBar", () => {
  it("renders scope buttons, search, and filters", async () => {
    const { TeamTaskFiltersBar } = await import("@/components/dashboard/team-task/TeamTaskFiltersBar");
    render(
      <TeamTaskFiltersBar
        scope="all"
        onScopeChange={vi.fn()}
    ...
    expect(screen.getByText("I Assigned")).toBeInTheDocument();
    expect(screen.getByText("Collaborative")).toBeInTheDocument();
    expect(screen.getByPlaceholderText("Search team tasks...")).toBeInTheDocument();
    expect(screen.getByText("Needs Attention")).toBeInTheDocument();
  });

  it("calls onScopeChange when scope button clicked", async () => {
    ...
    expect(onScopeChange).toHaveBeenCalledWith("assigned_to_me");
  });
});
