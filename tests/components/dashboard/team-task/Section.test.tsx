...
describe("Section (TeamTask)", () => {
  it("renders title and children", async () => {
    const { Section } = await import("@/components/dashboard/team-task/Section");
    render(
  ...
  it("applies highlight class", async () => {
    const { Section } = await import("@/components/dashboard/team-task/Section");
    render(
      <Section title="Urgent" highlight>
        <div>Content</div>
      </Section>
    );
    expect(screen.getByText("Urgent").className).toContain("text-destructive");
  });
});
