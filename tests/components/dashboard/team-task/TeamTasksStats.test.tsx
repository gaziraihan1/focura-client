...
describe("TeamTasksStats", () => {
  it("renders stat cards with values", async () => {
    const { TeamTasksStats } = await import("@/components/dashboard/team-task/TeamTasksStats");
    render(
      <TeamTasksStats stats={{ assigned: 10, overdue: 3, dueToday: 2, inProgress: 5 }} />
    );
  ...
  it("renders with null stats", async () => {
    const { TeamTasksStats } = await import("@/components/dashboard/team-task/TeamTasksStats");
    render(<TeamTasksStats stats={null} />);
