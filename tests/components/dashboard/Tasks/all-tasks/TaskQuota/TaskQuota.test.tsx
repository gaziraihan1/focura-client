...
describe('AllTasks/TaskQuota – ProgressRing', () => {
  it('renders SVG circles', async () => {
    const { ProgressRing } = await import('@/components/dashboard/tasks/all-tasks/TaskQuota/ProgressRing')
    const { container } = render(<ProgressRing used={5} limit={10} />)
    expect(container.querySelectorAll('circle').length).toBe(2)
  })
  it('applies destructive color at 90%', async () => {
    const { ProgressRing } = await import('@/components/dashboard/tasks/all-tasks/TaskQuota/ProgressRing')
    const { container } = render(<ProgressRing used={9} limit={10} />)
    const circles = container.querySelectorAll('circle')
    expect(circles[1]).toHaveClass('stroke-destructive')
  })
  it('applies chart-5 color at 70%', async () => {
    const { ProgressRing } = await import('@/components/dashboard/tasks/all-tasks/TaskQuota/ProgressRing')
    const { container } = render(<ProgressRing used={7} limit={10} />)
    const circles = container.querySelectorAll('circle')
    expect(circles[1]).toHaveClass('stroke-chart-5')
  })
  it('applies primary color below 70%', async () => {
    const { ProgressRing } = await import('@/components/dashboard/tasks/all-tasks/TaskQuota/ProgressRing')
    const { container } = render(<ProgressRing used={3} limit={10} />)
    const circles = container.querySelectorAll('circle')
    expect(circles[1]).toHaveClass('stroke-primary')
  })
  it('applies chart-2 for unlimited', async () => {
    const { ProgressRing } = await import('@/components/dashboard/tasks/all-tasks/TaskQuota/ProgressRing')
    const { container } = render(<ProgressRing used={5} limit={10} isUnlimited={true} />)
    const circles = container.querySelectorAll('circle')
    expect(circles[1]).toHaveClass('stroke-chart-2')
  })
})

describe('AllTasks/TaskQuota – QuotaSkeleton', () => {
  it('renders skeleton', async () => {
    const { QuotaSkeleton } = await import('@/components/dashboard/tasks/all-tasks/TaskQuota/QuotaSkeleton')
    const { container } = render(<QuotaSkeleton />)
    expect(container.querySelector('.animate-pulse')).toBeInTheDocument()
  })
})

describe('AllTasks/TaskQuota – StartPill (StatPill)', () => {
  it('renders label and value', async () => {
    const { StatPill } = await import('@/components/dashboard/tasks/all-tasks/TaskQuota/StartPill')
    render(<StatPill label="Tasks" value={5} />)
    expect(screen.getByText('Tasks')).toBeInTheDocument()
    expect(screen.getByText('5')).toBeInTheDocument()
  })
  it('renders sub text', async () => {
    const { StatPill } = await import('@/components/dashboard/tasks/all-tasks/TaskQuota/StartPill')
    render(<StatPill label="Rate" value="80%" sub="high" />)
