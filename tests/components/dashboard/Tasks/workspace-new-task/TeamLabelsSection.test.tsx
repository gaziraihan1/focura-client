...
describe('WorkspaceNewTask/TeamLabelsSection', () => {
  it('renders team & labels header', async () => {
    const { TeamLabelsSection } = await import('@/components/dashboard/tasks/workspace-new-task/TeamLabelsSection')
    render(
      <TeamLabelsSection
        members={[{ user: { id: 'u1', name: 'Alice' } }]} membersLoading={false}
        assigneeIds={[]} labelIds={[]} workspaceId="w1"
        canAssignToOthers={true} onToggleAssignee={vi.fn()}
        onLabelChange={vi.fn()} onOpenLabelManager={vi.fn()}
      />
    )
    expect(screen.getByText('Team & Labels')).toBeInTheDocument()
  })
  ...
  it('shows no members message', async () => {
    const { TeamLabelsSection } = await import('@/components/dashboard/tasks/workspace-new-task/TeamLabelsSection')
    render(
      <TeamLabelsSection
  ...
  it('shows self-only hint when cannotAssignToOthers', async () => {
    const { TeamLabelsSection } = await import('@/components/dashboard/tasks/workspace-new-task/TeamLabelsSection')
    render(
      <TeamLabelsSection
        members={[{ user: { id: 'u1', name: 'Alice' } }]} membersLoading={false}
        assigneeIds={[]} labelIds={[]} workspaceId="w1"
        canAssignToOthers={false} onToggleAssignee={vi.fn()}
        onLabelChange={vi.fn()} onOpenLabelManager={vi.fn()}
      />
    )
    expect(screen.getByText(/can only assign to yourself/)).toBeInTheDocument()
  })
  it('calls onOpenLabelManager', async () => {
    const onOpenLabelManager = vi.fn()
    const { TeamLabelsSection } = await import('@/components/dashboard/tasks/workspace-new-task/TeamLabelsSection')
    render(
      <TeamLabelsSection
        members={[]} membersLoading={false}
        assigneeIds={[]} labelIds={[]} workspaceId="w1"
        canAssignToOthers={true} onToggleAssignee={vi.fn()}
        onLabelChange={vi.fn()} onOpenLabelManager={onOpenLabelManager}
      />
    )
    fireEvent.click(screen.getByText('Manage Labels'))
    expect(onOpenLabelManager).toHaveBeenCalled()
  })
  it('renders label picker', async () => {
    const { TeamLabelsSection } = await import('@/components/dashboard/tasks/workspace-new-task/TeamLabelsSection')
    render(
      <TeamLabelsSection
        members={[]} membersLoading={false}
  ...
  it('shows project restriction message', async () => {
    const { TeamLabelsSection } = await import('@/components/dashboard/tasks/workspace-new-task/TeamLabelsSection')
    render(
      <TeamLabelsSection
        members={[{ user: { id: 'u1', name: 'Alice' } }]} membersLoading={false}
        assigneeIds={[]} labelIds={[]} workspaceId="w1" projectId="p1"
        canAssignToOthers={false} currentUserId="u2" onToggleAssignee={vi.fn()}
        onLabelChange={vi.fn()} onOpenLabelManager={vi.fn()}
      />
    )
    expect(screen.getByText(/project manager or workspace admin/)).toBeInTheDocument()
  })
})
