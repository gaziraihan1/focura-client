...
describe('WorkspaceNewTask/FormActions', () => {
  it('renders Create Task button', async () => {
    const { FormActions } = await import('@/components/dashboard/tasks/workspace-new-task/FormActions')
    render(<FormActions isLoading={false} canSubmit={true} onCancel={vi.fn()} />)
  ...
  it('shows Creating... when loading', async () => {
    const { FormActions } = await import('@/components/dashboard/tasks/workspace-new-task/FormActions')
    render(<FormActions isLoading={true} canSubmit={true} onCancel={vi.fn()} />)
    expect(screen.getByText('Creating...')).toBeInTheDocument()
  })
  it('calls onCancel', async () => {
    const onCancel = vi.fn()
    const { FormActions } = await import('@/components/dashboard/tasks/workspace-new-task/FormActions')
    render(<FormActions isLoading={false} canSubmit={true} onCancel={onCancel} />)
    fireEvent.click(screen.getByText('Cancel'))
    expect(onCancel).toHaveBeenCalled()
  })
  it('disables submit when canSubmit is false', async () => {
    const { FormActions } = await import('@/components/dashboard/tasks/workspace-new-task/FormActions')
    render(<FormActions isLoading={false} canSubmit={false} onCancel={vi.fn()} />)
