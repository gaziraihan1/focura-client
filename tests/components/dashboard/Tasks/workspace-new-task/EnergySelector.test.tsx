...
describe('WorkspaceNewTask/EnergySelector', () => {
  it('renders energy options', async () => {
    const { EnergySelector } = await import('@/components/dashboard/tasks/workspace-new-task/EnergySelector')
    render(<EnergySelector selectedEnergy="LOW" onEnergyChange={vi.fn()} />)
    expect(screen.getByText('Low Energy')).toBeInTheDocument()
    expect(screen.getByText('Medium')).toBeInTheDocument()
    expect(screen.getByText('High Focus')).toBeInTheDocument()
  })
  it('calls onEnergyChange on click', async () => {
    const onEnergyChange = vi.fn()
    const { EnergySelector } = await import('@/components/dashboard/tasks/workspace-new-task/EnergySelector')
    render(<EnergySelector selectedEnergy="LOW" onEnergyChange={onEnergyChange} />)
    fireEvent.click(screen.getByText('High Focus'))
