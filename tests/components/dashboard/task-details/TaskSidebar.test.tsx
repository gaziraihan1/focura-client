...
describe('TaskDetails/TaskSidebar – StatusSelector', () => {
  it('renders lock when cannot change status', async () => {
    const { StatusSelector } = await import('@/components/dashboard/task-details/TaskSidebar/StatusSelector')
    render(<StatusSelector status="TODO" isPersonalTask={false} isUpdatingStatus={false} canChangeStatus={false} onStatusChange={vi.fn()} />)
    expect(screen.getByText(/don't have permission/)).toBeInTheDocument()
  })
  it('renders select when can change', async () => {
    const { StatusSelector } = await import('@/components/dashboard/task-details/TaskSidebar/StatusSelector')
    render(<StatusSelector status="TODO" isPersonalTask={false} isUpdatingStatus={false} canChangeStatus={true} onStatusChange={vi.fn()} />)
    expect(screen.getByDisplayValue('To Do')).toBeInTheDocument()
  })
  it('shows personal task hint', async () => {
    const { StatusSelector } = await import('@/components/dashboard/task-details/TaskSidebar/StatusSelector')
    render(<StatusSelector status="TODO" isPersonalTask={true} isUpdatingStatus={false} canChangeStatus={true} onStatusChange={vi.fn()} />)
...
describe('TaskDetails/TaskSidebar – TaskDetailItem', () => {
  it('renders label and value', async () => {
    const { TaskDetailItem } = await import('@/components/dashboard/task-details/TaskSidebar/TaskDetailItem')
...
describe('TaskDetails/TaskSidebar – TaskAssigneesList', () => {
  it('returns null when no assignees', async () => {
    const { TaskAssigneesList } = await import('@/components/dashboard/task-details/TaskSidebar/TaskAssigneesList')
    const { container } = render(<TaskAssigneesList assignees={[]} />)
    expect(container.innerHTML).toBe('')
  })
  it('renders assignees', async () => {
    const { TaskAssigneesList } = await import('@/components/dashboard/task-details/TaskSidebar/TaskAssigneesList')
    render(<TaskAssigneesList assignees={[{ user: { id: 'u1', name: 'Alice', image: null } }]} />)
    expect(screen.getByText('Assignees')).toBeInTheDocument()
    expect(screen.getByText('Alice')).toBeInTheDocument()
  })
})

describe('TaskDetails/TaskSidebar – TaskTimestamps', () => {
  it('renders created date', async () => {
    const { TaskTimestamps } = await import('@/components/dashboard/task-details/TaskSidebar/TaskTimeStamps')
    render(<TaskTimestamps createdAt="2025-01-01T00:00:00Z" />)
    expect(screen.getByText(/Created/)).toBeInTheDocument()
  })
  it('renders completed date when present', async () => {
    const { TaskTimestamps } = await import('@/components/dashboard/task-details/TaskSidebar/TaskTimeStamps')
    render(<TaskTimestamps createdAt="2025-01-01T00:00:00Z" completedAt="2025-01-05T00:00:00Z" />)
    expect(screen.getByText(/Completed/)).toBeInTheDocument()
  })
  it('hides completed date when absent', async () => {
    const { TaskTimestamps } = await import('@/components/dashboard/task-details/TaskSidebar/TaskTimeStamps')
    render(<TaskTimestamps createdAt="2025-01-01T00:00:00Z" />)
...
describe('TaskDetails/TaskSidebar – PriorityDisplay', () => {
  it('renders priority', async () => {
    const { PriorityDisplay } = await import('@/components/dashboard/task-details/TaskSidebar/PriorityDisplay')
    render(<PriorityDisplay priority="HIGH" />)
    expect(screen.getByText('HIGH')).toBeInTheDocument()
  })
  it('renders Priority label', async () => {
    const { PriorityDisplay } = await import('@/components/dashboard/task-details/TaskSidebar/PriorityDisplay')
    render(<PriorityDisplay priority="LOW" />)
...
describe('TaskDetails/TaskSidebar – main TaskSidebar', () => {
  it('renders Details header', async () => {
    const { TaskSidebar } = await import('@/components/dashboard/task-details/TaskSidebar')
