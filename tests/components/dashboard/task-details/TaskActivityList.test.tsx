...
describe('TaskDetails/TaskActivityList', () => {
  it('renders EmptyState when activities is empty', async () => {
    const { TaskActivityList } = await import('@/components/dashboard/task-details/TaskActivityList')
    render(<TaskActivityList activities={[]} />)
    expect(screen.getByText('No activity yet')).toBeInTheDocument()
  })
  it('renders grouped activities when present', async () => {
    const { TaskActivityList } = await import('@/components/dashboard/task-details/TaskActivityList')
    const act = {
      id: 'a1', action: 'UPDATED', entityType: 'TASK',
      createdAt: new Date().toISOString(),
      user: { id: 'u1', name: 'Alice', image: null }, metadata: {},
    }
    render(<TaskActivityList activities={[act]} />)
    expect(screen.getByText('updated this task')).toBeInTheDocument()
  })
})
