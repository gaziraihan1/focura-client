...
describe('TaskDetails/FocusRequirementsCard', () => {
  it('renders focus level', async () => {
    const { FocusRequirementsCard } = await import('@/components/dashboard/task-details/FocusRequirementsCard')
    render(<FocusRequirementsCard focusLevel={4} energyType="HIGH" distractionCost={3} />)
    expect(screen.getByText('4/5')).toBeInTheDocument()
  })
  it('renders energy type', async () => {
    const { FocusRequirementsCard } = await import('@/components/dashboard/task-details/FocusRequirementsCard')
    render(<FocusRequirementsCard focusLevel={4} energyType="HIGH" distractionCost={3} />)
    expect(screen.getByText('HIGH')).toBeInTheDocument()
  })
  it('renders distraction cost', async () => {
    const { FocusRequirementsCard } = await import('@/components/dashboard/task-details/FocusRequirementsCard')
    render(<FocusRequirementsCard focusLevel={4} energyType="HIGH" distractionCost={3} />)
    expect(screen.getByText('3/5')).toBeInTheDocument()
  })
  it('hides focus level when not provided', async () => {
    const { FocusRequirementsCard } = await import('@/components/dashboard/task-details/FocusRequirementsCard')
    render(<FocusRequirementsCard energyType="HIGH" />)
    expect(screen.queryByText(/Focus Level/)).not.toBeInTheDocument()
  })
  it('hides energy type when not provided', async () => {
    const { FocusRequirementsCard } = await import('@/components/dashboard/task-details/FocusRequirementsCard')
    render(<FocusRequirementsCard focusLevel={3} />)
    expect(screen.queryByText('HIGH')).not.toBeInTheDocument()
  })
  it('hides distraction cost when not provided', async () => {
    const { FocusRequirementsCard } = await import('@/components/dashboard/task-details/FocusRequirementsCard')
    const { container } = render(<FocusRequirementsCard energyType="LOW" />)
    expect(container.textContent).not.toContain('Distraction Cost')
  })
  it('renders warning message', async () => {
    const { FocusRequirementsCard } = await import('@/components/dashboard/task-details/FocusRequirementsCard')
    render(<FocusRequirementsCard focusLevel={4} />)
