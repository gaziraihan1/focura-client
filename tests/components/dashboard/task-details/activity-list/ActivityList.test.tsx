...
describe('TaskDetails/ActivityList – EmptyState', () => {
  it('renders no activity message', async () => {
    ...
    render(
      <EmptyState icon={Clock} title="No activity yet" description="Activity will appear here when tasks change." />
    )
...
describe('TaskDetails/ActivityList – ActivityIcon', () => {
  it('renders Plus for "Plus"', async () => {
    const { ActivityIcon } = await import('@/components/dashboard/task-details/ActivityList/ActivityIcon')
    render(<ActivityIcon iconName="Plus" />)
    expect(screen.getByTestId('icon-Plus')).toBeInTheDocument()
  })
  it('renders Pencil for "Pencil"', async () => {
    const { ActivityIcon } = await import('@/components/dashboard/task-details/ActivityList/ActivityIcon')
    render(<ActivityIcon iconName="Pencil" />)
    expect(screen.getByTestId('icon-Pencil')).toBeInTheDocument()
  })
  it('renders Trash2 for "Trash2"', async () => {
    const { ActivityIcon } = await import('@/components/dashboard/task-details/ActivityList/ActivityIcon')
    render(<ActivityIcon iconName="Trash2" />)
    expect(screen.getByTestId('icon-Trash2')).toBeInTheDocument()
  })
  it('renders CheckCircle2 for "CheckCircle2"', async () => {
    const { ActivityIcon } = await import('@/components/dashboard/task-details/ActivityList/ActivityIcon')
    render(<ActivityIcon iconName="CheckCircle2" />)
    expect(screen.getByTestId('icon-CheckCircle2')).toBeInTheDocument()
  })
  it('renders Users for "Users"', async () => {
    const { ActivityIcon } = await import('@/components/dashboard/task-details/ActivityList/ActivityIcon')
    render(<ActivityIcon iconName="Users" />)
    expect(screen.getByTestId('icon-Users')).toBeInTheDocument()
  })
  it('renders MessageSquare for "MessageSquare"', async () => {
    const { ActivityIcon } = await import('@/components/dashboard/task-details/ActivityList/ActivityIcon')
    render(<ActivityIcon iconName="MessageSquare" />)
    expect(screen.getByTestId('icon-MessageSquare')).toBeInTheDocument()
  })
  it('renders AlertCircle for "AlertCircle"', async () => {
    const { ActivityIcon } = await import('@/components/dashboard/task-details/ActivityList/ActivityIcon')
    render(<ActivityIcon iconName="AlertCircle" />)
    expect(screen.getByTestId('icon-AlertCircle')).toBeInTheDocument()
  })
  it('renders Circle for unknown icon name', async () => {
    const { ActivityIcon } = await import('@/components/dashboard/task-details/ActivityList/ActivityIcon')
    render(<ActivityIcon iconName="UnknownIcon" />)
    expect(screen.getByTestId('icon-Circle')).toBeInTheDocument()
  })
  it('applies custom className', async () => {
    const { ActivityIcon } = await import('@/components/dashboard/task-details/ActivityList/ActivityIcon')
    render(<ActivityIcon iconName="Plus" className="h-8 w-8" />)
    expect(screen.getByTestId('icon-Plus')).toHaveClass('h-8', 'w-8')
  })
})

describe('TaskDetails/ActivityList – UserAvatar', () => {
  it('renders avatar with user name', async () => {
    const { UserAvatar } = await import('@/components/dashboard/task-details/ActivityList/UserAvatar')
    render(<UserAvatar user={{ id: 'u1', name: 'Bob', image: null }} />)
    expect(screen.getByTestId('avatar')).toHaveAttribute('data-name', 'Bob')
  })
})

describe('TaskDetails/ActivityList – ActivityItem', () => {
  const baseActivity = {
    id: 'act-1', action: 'CREATED', entityType: 'TASK',
    createdAt: new Date().toISOString(),
    user: { id: 'user-1', name: 'Alice', image: null },
    metadata: {},
  }
  it('renders user name', async () => {
    const { ActivityItem } = await import('@/components/dashboard/task-details/ActivityList/ActivityItem')
  ...
  it('renders subtaskTitle from metadata', async () => {
    const { ActivityItem } = await import('@/components/dashboard/task-details/ActivityList/ActivityItem')
    render(<ActivityItem activity={{ ...baseActivity, metadata: { subtaskTitle: 'Fix bug' } }} />)
    expect(screen.getByText('Fix bug')).toBeInTheDocument()
  })
  it('renders taskTitle from metadata fallback', async () => {
    const { ActivityItem } = await import('@/components/dashboard/task-details/ActivityList/ActivityItem')
    render(<ActivityItem activity={{ ...baseActivity, metadata: { taskTitle: 'Feature X' } }} />)
    expect(screen.getByText('Feature X')).toBeInTheDocument()
  })
})

describe('TaskDetails/ActivityList – DateGroup', () => {
  it('renders date header', async () => {
    const { DateGroup } = await import('@/components/dashboard/task-details/ActivityList/DateGroup')
    render(<DateGroup date="January 1, 2025" activities={[]} />)
    expect(screen.getByText('January 1, 2025')).toBeInTheDocument()
  })
  it('renders activities in the group', async () => {
    const { DateGroup } = await import('@/components/dashboard/task-details/ActivityList/DateGroup')
    const act = {
      id: 'a1', action: 'CREATED', entityType: 'TASK',
      createdAt: new Date().toISOString(),
      user: { id: 'u1', name: 'Bob', image: null }, metadata: {},
    }
    render(<DateGroup date="Jan 1" activities={[act]} />)
    expect(screen.getByText('created this task')).toBeInTheDocument()
  })
})
