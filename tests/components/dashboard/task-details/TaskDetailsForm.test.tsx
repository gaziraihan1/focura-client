...
describe('TaskDetails/TaskDetailsForm', () => {
  ...
  it('renders title input', async () => {
    const { TaskDetailsForm } = await import('@/components/dashboard/task-details/TaskDetailsForm')
  ...
  it('shows loader when updating', async () => {
    const { TaskDetailsForm } = await import('@/components/dashboard/task-details/TaskDetailsForm')
    render(<TaskDetailsForm editData={editData} isUpdating={true} onEditDataChange={vi.fn()} onSave={vi.fn()} onCancel={vi.fn()} />)
