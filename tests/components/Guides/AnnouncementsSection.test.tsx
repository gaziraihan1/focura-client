...
import { AnnouncementsSection } from '@/components/Guides/AnnouncementSection'

describe('AnnouncementsSection', () => {
  it('renders "What are announcements?"', () => {
    render(<AnnouncementsSection />)
    expect(screen.getByText('What are announcements?')).toBeInTheDocument()
  })

  it('renders creation steps', () => {
    render(<AnnouncementsSection />)
    expect(screen.getByText('Go to Announcements')).toBeInTheDocument()
    expect(screen.getByText('Write your message')).toBeInTheDocument()
    expect(screen.getByText('Set visibility')).toBeInTheDocument()
    expect(screen.getByText('Pin if important')).toBeInTheDocument()
  })

  it('renders the live preview info', () => {
    render(<AnnouncementsSection />)
    expect(screen.getByText('See it before you send it')).toBeInTheDocument()
  })
})
