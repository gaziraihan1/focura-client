...
import { TasksSection } from '@/components/Guides/TaskSection'

describe('TasksSection', () => {
  it('renders task creation steps', () => {
    render(<TasksSection />)
    expect(screen.getByText('Open a project')).toBeInTheDocument()
    expect(screen.getByText(/Click "New Task"/)).toBeInTheDocument()
    expect(screen.getByText('Fill in task details')).toBeInTheDocument()
    expect(screen.getByText('Save and track')).toBeInTheDocument()
  })
  ...
  it('renders subtasks section', () => {
    render(<TasksSection />)
    expect(screen.getByText('Breaking work into smaller pieces')).toBeInTheDocument()
  })

  it('renders attachments section', () => {
    render(<TasksSection />)
    expect(screen.getByText('Attachments & comments')).toBeInTheDocument()
  })
})
