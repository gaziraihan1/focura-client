...
describe('useSectionContent', () => {
  it('returns content for known section IDs', () => {
    const result = useSectionContent('getting-started')
    expect(result).not.toBeNull()
  })

  it('returns content for workspace', () => {
    const result = useSectionContent('workspace')
    expect(result).not.toBeNull()
  })

  it('returns null for unknown section ID', () => {
    const result = useSectionContent('nonexistent')
    expect(result).toBeNull()
  })

  it('returns content for all mapped sections', () => {
    const ids = [
      'getting-started', 'workspace', 'projects', 'tasks',
      'announcements', 'members', 'invite-members', 'labels',
      'activity', 'focus', 'meetings', 'voting', 'billing',
    ]
    ids.forEach((id) => {
      expect(useSectionContent(id)).not.toBeNull()
    })
  })
})
