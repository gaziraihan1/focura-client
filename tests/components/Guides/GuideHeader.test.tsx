...
describe('GuideHeader', () => {
  const current = guideSections[0]

  it('renders the Focura logo', () => {
    render(<GuideHeader current={current} mobileOpen={false} onMobileToggle={vi.fn()} />)
  ...
  it('renders current section label', () => {
    render(<GuideHeader current={current} mobileOpen={false} onMobileToggle={vi.fn()} />)
    const labels = screen.getAllByText('Getting Started')
    expect(labels.length).toBeGreaterThanOrEqual(1)
  })

  it('calls onMobileToggle when mobile button clicked', () => {
    const toggle = vi.fn()
    render(<GuideHeader current={current} mobileOpen={false} onMobileToggle={toggle} />)
