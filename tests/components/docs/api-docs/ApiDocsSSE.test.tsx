...
describe("ApiDocsSSE", () => {
  it("renders SSE guide with event types", async () => {
    const { ApiDocsSSE } = await import("@/components/docs/api-docs/ApiDocsSSE");
    render(<ApiDocsSSE />);
    expect(screen.getByText("Real-time Events (SSE)")).toBeInTheDocument();
    expect(screen.getByText("TASK_ASSIGNED")).toBeInTheDocument();
    expect(screen.getByText("ANNOUNCEMENT")).toBeInTheDocument();
    expect(screen.getByText("SSE Event Shape")).toBeInTheDocument();
    expect(screen.getByText("React + TanStack Query Integration")).toBeInTheDocument();
  });
});
