...
describe("ApiDocsEndpointCardParts", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  afterEach(() => {
  ...
  it("CopyButton copies text and shows check", async () => {
    const { CopyButton } = await import("@/components/docs/api-docs/ApiDocsEndpointCardParts");
  ...
  it("ParamsTable renders required and optional params", async () => {
    const { ParamsTable } = await import("@/components/docs/api-docs/ApiDocsEndpointCardParts");
    render(
      <ParamsTable
        title="Path Params"
        params={[
          { name: "id", type: "string", required: true, description: "User ID", example: "abc123" },
          { name: "filter", type: "string", required: false, description: "Filter string" },
        ]}
      />
    );
    expect(screen.getByText("Path Params")).toBeInTheDocument();
  ...
  it("ResponseList renders status codes with colors", async () => {
    const { ResponseList } = await import("@/components/docs/api-docs/ApiDocsEndpointCardParts");
    render(
      <ResponseList
        responses={[
          { status: 200, description: "OK", shape: [{ name: "data", type: "object", description: "Result" }] },
  ...
  it("CodeExamples switches tabs and copies", async () => {
    const { CodeExamples } = await import("@/components/docs/api-docs/ApiDocsEndpointCardParts");
    const writeText = vi.fn().mockResolvedValue(undefined);
    Object.assign(navigator, { clipboard: { writeText } });
    render(
      <CodeExamples
        examples={[
          { label: "cURL", code: "curl /api" },
          { label: "fetch", code: "fetch('/api')" },
        ]}
      />
    );
    expect(screen.getByText("curl /api")).toBeInTheDocument();
