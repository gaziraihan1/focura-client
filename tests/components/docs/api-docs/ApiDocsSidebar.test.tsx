...
describe("ApiDocsSidebar", () => {
  it("renders overview sections and endpoint sections", async () => {
    const { ApiDocsSidebar } = await import("@/components/docs/api-docs/ApiDocsSidebar");
    render(
      <ApiDocsSidebar
        activeSection="workspace-endpoints"
        activeEndpoint=""
    ...
    expect(screen.getByText("List all workspaces")).toBeInTheDocument();
  });

  it("calls onSectionClick and onEndpointClick", async () => {
    ...
    render(
      <ApiDocsSidebar
        activeSection="workspace-endpoints"
        activeEndpoint=""
        onSectionClick={onSection}
        onEndpointClick={onEndpoint}
      />
    );
    fireEvent.click(screen.getByText("Authentication"));
    expect(onSection).toHaveBeenCalledWith("authentication-guide");
    fireEvent.click(screen.getByText("List all workspaces"));
    expect(onEndpoint).toHaveBeenCalledWith("workspace-endpoints", "list-workspaces");
  });
});
