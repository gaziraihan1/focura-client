...
describe("ApiDocsAuthentication", () => {
  it("renders authentication steps and security notes", async () => {
    const { ApiDocsAuthentication } = await import("@/components/docs/api-docs/ApiDocsAuthentication");
    render(<ApiDocsAuthentication />);
    expect(screen.getByText("Authentication")).toBeInTheDocument();
    expect(screen.getByText("Obtain an access token")).toBeInTheDocument();
    expect(screen.getByText("Attach the token to requests")).toBeInTheDocument();
    expect(screen.getByText("Silent refresh before expiry")).toBeInTheDocument();
    expect(screen.getByText("Handle 401 responses")).toBeInTheDocument();
    expect(screen.getByText("JWT Access Token Payload")).toBeInTheDocument();
  });
});
