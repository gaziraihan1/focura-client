...
import { PrivacyList } from '@/components/PrivacyAndPolicy/PrivacyList'

describe('PrivacyList', () => {
  it('renders an unordered list by default', () => {
    const { container } = render(<PrivacyList items={['Item 1', 'Item 2']} />)
    const list = container.querySelector('ul')
    expect(list).toBeInTheDocument()
  })

  it('renders an ordered list when ordered prop is true', () => {
    const { container } = render(<PrivacyList items={['Step 1', 'Step 2']} ordered />)
    const list = container.querySelector('ol')
    expect(list).toBeInTheDocument()
  })

  it('renders all items', () => {
    render(<PrivacyList items={['Alpha', 'Beta', 'Gamma']} />)
    expect(screen.getByText('Alpha')).toBeInTheDocument()
    expect(screen.getByText('Beta')).toBeInTheDocument()
    expect(screen.getByText('Gamma')).toBeInTheDocument()
  })

  it('renders numbered indicators for ordered lists', () => {
    render(<PrivacyList items={['First', 'Second']} ordered />)
