...
describe('ActivityLogs/ActivityContent', () => {
  it('shows loading state', async () => {
    const { ActivityContent } = await import('@/components/dashboard/activity-logs/ActivityContent')
    const { container } = render(<ActivityContent activities={undefined} isLoading={true} error={null} />)
    expect(container.querySelector('.animate-pulse, [class*="animate"]')).toBeInTheDocument()
  })
  it('shows error state', async () => {
    const { ActivityContent } = await import('@/components/dashboard/activity-logs/ActivityContent')
    render(<ActivityContent activities={undefined} isLoading={false} error={new Error('fail')} />)
    expect(screen.getByText(/Failed to load/)).toBeInTheDocument()
  })
  it('shows empty state', async () => {
    const { ActivityContent } = await import('@/components/dashboard/activity-logs/ActivityContent')
    render(<ActivityContent activities={[]} isLoading={false} error={null} />)
    expect(screen.getByText(/No activities/)).toBeInTheDocument()
  })
  it('shows activity list', async () => {
    const { ActivityContent } = await import('@/components/dashboard/activity-logs/ActivityContent')
    const act = {
    ...
    render(<ActivityContent activities={[act]} isLoading={false} error={null} />)
