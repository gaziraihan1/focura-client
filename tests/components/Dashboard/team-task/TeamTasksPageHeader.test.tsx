...
describe("TeamTasksPageHeader", () => {
  it("renders with workspace id", async () => {
    const { TeamTasksPageHeader } = await import("@/components/dashboard/team-task/TeamTasksPageHeader");
    render(<TeamTasksPageHeader workspaceId="ws1" />);
  ...
  it("renders without workspace id", async () => {
    const { TeamTasksPageHeader } = await import("@/components/dashboard/team-task/TeamTasksPageHeader");
    render(<TeamTasksPageHeader />);
    expect(screen.getByText("Work that requires coordination and shared accountability")).toBeInTheDocument();
  });
});
