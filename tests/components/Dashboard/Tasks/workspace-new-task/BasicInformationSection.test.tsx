...
describe('WorkspaceNewTask/BasicInformationSection', () => {
  it('renders title input', async () => {
    const { BasicInformationSection } = await import('@/components/dashboard/tasks/workspace-new-task/BasicInformationSection')
    render(
      <BasicInformationSection
        title="" description="" projects={[]} projectsLoading={false}
        errors={{}} workspaceSlug="ws" onTitleChange={vi.fn()}
        onDescriptionChange={vi.fn()} onProjectChange={vi.fn()}
      />
    )
    expect(screen.getByText('Basic Information')).toBeInTheDocument()
    expect(screen.getByPlaceholderText('Enter task title...')).toBeInTheDocument()
  })
  it('shows loading state for projects', async () => {
    const { BasicInformationSection } = await import('@/components/dashboard/tasks/workspace-new-task/BasicInformationSection')
    render(
      <BasicInformationSection
        title="" description="" projects={[]} projectsLoading={true}
        errors={{}} workspaceSlug="ws" onTitleChange={vi.fn()}
        onDescriptionChange={vi.fn()} onProjectChange={vi.fn()}
      />
    )
  ...
  it('shows project select when projects exist', async () => {
    const { BasicInformationSection } = await import('@/components/dashboard/tasks/workspace-new-task/BasicInformationSection')
    render(
      <BasicInformationSection
        title="" description="" projects={[{ id: 'p1', name: 'Project A' }]} projectsLoading={false}
        errors={{}} workspaceSlug="ws" onTitleChange={vi.fn()}
        onDescriptionChange={vi.fn()} onProjectChange={vi.fn()}
      />
    )
  ...
  it('shows title error', async () => {
    ...
    render(
      <BasicInformationSection
        title="" description="" projects={[]} projectsLoading={false}
        errors={{ title: 'Title is required' }} workspaceSlug="ws" onTitleChange={vi.fn()}
        onDescriptionChange={vi.fn()} onProjectChange={vi.fn()}
      />
    )
    expect(screen.getByText('Title is required')).toBeInTheDocument()
  })
  it('shows project error', async () => {
    const { BasicInformationSection } = await import('@/components/dashboard/tasks/workspace-new-task/BasicInformationSection')
    render(
      <BasicInformationSection
        title="" description="" projects={[{ id: 'p1', name: 'P' }]} projectsLoading={false}
        errors={{ projectId: 'Select a project' }} workspaceSlug="ws" onTitleChange={vi.fn()}
        onDescriptionChange={vi.fn()} onProjectChange={vi.fn()}
      />
    )
    expect(screen.getAllByText('Select a project').length).toBeGreaterThanOrEqual(2)
  })
})
