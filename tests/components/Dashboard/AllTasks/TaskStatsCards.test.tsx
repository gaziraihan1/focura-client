...
describe('AllTasks/TaskStatsCards', () => {
  it('renders all tab cards', async () => {
    const { TaskStatsCards } = await import('@/components/Dashboard/AllTasks/TaskStatsCards')
    render(<TaskStatsCards stats={{ personal: 2, assigned: 3, dueToday: 1, overdue: 0, totalTasks: 5, inProgress: 2, completed: 3 }} activeTab="all" />)
    expect(screen.getAllByTestId('stat-card').length).toBeGreaterThan(0)
  })
  it('renders personal tab cards', async () => {
    const { TaskStatsCards } = await import('@/components/Dashboard/AllTasks/TaskStatsCards')
    render(<TaskStatsCards stats={{ personal: 2, assigned: 3, dueToday: 1, overdue: 0 }} activeTab="personal" />)
    expect(screen.getAllByTestId('stat-card').length).toBe(4)
  })
  it('renders assigned tab cards', async () => {
    const { TaskStatsCards } = await import('@/components/Dashboard/AllTasks/TaskStatsCards')
    render(<TaskStatsCards stats={{ personal: 2, assigned: 3, dueToday: 1, overdue: 0 }} activeTab="assigned" />)
    expect(screen.getAllByTestId('stat-card').length).toBe(4)
  })
})
