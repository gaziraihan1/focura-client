...
describe('AllTasks/FocusModeBanner', () => {
  const task = makeTask()
  it('renders FOCUS MODE ACTIVE', async () => {
    const { FocusModeBanner } = await import('@/components/Dashboard/AllTasks/FocusModeBanner')
    render(<FocusModeBanner task={task} timeRemaining={1500} onEndFocus={vi.fn()} />)
  ...
  it('calls onEndFocus on close button click', async () => {
    const onEndFocus = vi.fn()
    const { FocusModeBanner } = await import('@/components/Dashboard/AllTasks/FocusModeBanner')
    render(<FocusModeBanner task={task} timeRemaining={1500} onEndFocus={onEndFocus} />)
    fireEvent.click(screen.getByTitle('End focus session'))
