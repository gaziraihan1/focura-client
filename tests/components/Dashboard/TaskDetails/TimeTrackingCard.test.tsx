...
describe('TaskDetails/TimeTrackingCard', () => {
  const timeTracking = {
    hoursSinceCreation: 24, hoursUntilDue: 48, timeProgress: 50,
    isOverdue: false, isDueToday: false,
  }
  it('renders elapsed time', async () => {
    const { TimeTrackingCard } = await import('@/components/Dashboard/TaskDetails/TimeTrackingCard')
    render(<TimeTrackingCard timeTracking={timeTracking} estimatedHours={40} />)
  ...
  it('renders overdue badge', async () => {
    const { TimeTrackingCard } = await import('@/components/Dashboard/TaskDetails/TimeTrackingCard')
    render(<TimeTrackingCard timeTracking={{ ...timeTracking, isOverdue: true }} estimatedHours={40} />)
    expect(screen.getByText('Overdue')).toBeInTheDocument()
  })
  it('renders due today badge', async () => {
    const { TimeTrackingCard } = await import('@/components/Dashboard/TaskDetails/TimeTrackingCard')
    render(<TimeTrackingCard timeTracking={{ ...timeTracking, isDueToday: true }} estimatedHours={40} />)
    expect(screen.getByText('Due Today')).toBeInTheDocument()
  })
  it('hides progress bar without estimatedHours', async () => {
    const { TimeTrackingCard } = await import('@/components/Dashboard/TaskDetails/TimeTrackingCard')
