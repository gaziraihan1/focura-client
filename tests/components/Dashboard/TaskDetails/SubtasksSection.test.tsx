...
describe('TaskDetails/SubtasksSection – InlineEditor', () => {
  it('renders input with value', async () => {
    const { InlineEditor } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/InlineEditor')
    render(<InlineEditor value="Hello" onChange={vi.fn()} onSave={vi.fn()} onCancel={vi.fn()} isLoading={false} />)
    expect(screen.getByDisplayValue('Hello')).toBeInTheDocument()
  })
  it('calls onChange on input change', async () => {
    const onChange = vi.fn()
    const { InlineEditor } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/InlineEditor')
    render(<InlineEditor value="" onChange={onChange} onSave={vi.fn()} onCancel={vi.fn()} isLoading={false} />)
  ...
  it('calls onSave on Enter key', async () => {
    const onSave = vi.fn()
    const { InlineEditor } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/InlineEditor')
    render(<InlineEditor value="Test" onChange={vi.fn()} onSave={onSave} onCancel={vi.fn()} isLoading={false} />)
  ...
  it('calls onCancel on Escape key', async () => {
    ...
    const { InlineEditor } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/InlineEditor')
    render(<InlineEditor value="Test" onChange={vi.fn()} onSave={vi.fn()} onCancel={onCancel} isLoading={false} />)
    fireEvent.keyDown(screen.getByRole('textbox'), { key: 'Escape' })
    expect(onCancel).toHaveBeenCalled()
  })
  it('shows Loader2 when loading', async () => {
    const { InlineEditor } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/InlineEditor')
    render(<InlineEditor value="" onChange={vi.fn()} onSave={vi.fn()} onCancel={vi.fn()} isLoading={true} />)
    expect(screen.getByTestId('icon-Loader2')).toBeInTheDocument()
  })
})

describe('TaskDetails/SubtasksSection – StatusButton', () => {
  it('renders Circle for TODO', async () => {
    const { StatusButton } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/StatusButton')
    render(<StatusButton status="TODO" onChange={vi.fn()} disabled={false} />)
    expect(screen.getByTestId('icon-Circle')).toBeInTheDocument()
  })
  it('renders AlertCircle for IN_PROGRESS', async () => {
    const { StatusButton } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/StatusButton')
    render(<StatusButton status="IN_PROGRESS" onChange={vi.fn()} disabled={false} />)
    expect(screen.getByTestId('icon-AlertCircle')).toBeInTheDocument()
  })
  it('renders CheckCircle2 for COMPLETED', async () => {
    const { StatusButton } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/StatusButton')
    render(<StatusButton status="COMPLETED" onChange={vi.fn()} disabled={false} />)
    expect(screen.getByTestId('icon-CheckCircle2')).toBeInTheDocument()
  })
  it('calls onChange with next status on click', async () => {
    const onChange = vi.fn()
    const { StatusButton } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/StatusButton')
...
describe('TaskDetails/SubtasksSection – SubtaskList', () => {
  it('renders empty when no subtasks', async () => {
    const { SubtaskList } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskList')
    const { container } = render(
      <SubtaskList subtasks={[]} currentUserId="u1" isAssignee={true}
        onStatusChange={vi.fn()} onUpdate={vi.fn()} onDelete={vi.fn()} />
    )
    expect(container.firstChild).toBeTruthy()
  })
})

describe('TaskDetails/SubtasksSection – SubtaskEmptyState', () => {
  it('shows add button when canAdd is true', async () => {
    const { SubtaskEmptyState } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskEmptyState')
  ...
  it('renders different message for non-assignees', async () => {
    const { SubtaskEmptyState } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskEmptyState')
    render(<SubtaskEmptyState canAdd={false} onAdd={vi.fn()} />)
...
describe('TaskDetails/SubtasksSection – SubtaskProgress', () => {
  it('returns null when total is 0', async () => {
    const { SubtaskProgress } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskProgress')
    const { container } = render(<SubtaskProgress stats={{ total: 0, completed: 0, inProgress: 0, completionRate: 0 }} />)
    expect(container.innerHTML).toBe('')
  })
  it('renders progress when total > 0', async () => {
    const { SubtaskProgress } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskProgress')
    render(<SubtaskProgress stats={{ total: 5, completed: 3, inProgress: 1, completionRate: 60 }} />)
    expect(screen.getByText('3')).toBeInTheDocument()
    expect(screen.getByText('60%')).toBeInTheDocument()
  })
  it('renders 100% completion with special color', async () => {
    const { SubtaskProgress } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskProgress')
    render(<SubtaskProgress stats={{ total: 2, completed: 2, inProgress: 0, completionRate: 100 }} />)
    expect(screen.getByText('100%')).toBeInTheDocument()
  })
})

describe('TaskDetails/SubtasksSection – SubtaskForm', () => {
  it('renders title input', async () => {
    const { SubtaskForm } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskForm')
    render(<SubtaskForm onSubmit={vi.fn()} onCancel={vi.fn()} isLoading={false} />)
    expect(screen.getByPlaceholderText('Subtask title…')).toBeInTheDocument()
  })
  it('renders priority pills', async () => {
    const { SubtaskForm } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskForm')
    render(<SubtaskForm onSubmit={vi.fn()} onCancel={vi.fn()} isLoading={false} />)
  ...
  it('calls onCancel when cancel clicked', async () => {
    const onCancel = vi.fn()
    const { SubtaskForm } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskForm')
    render(<SubtaskForm onSubmit={vi.fn()} onCancel={onCancel} isLoading={false} />)
    fireEvent.click(screen.getByText('Cancel'))
    expect(onCancel).toHaveBeenCalled()
  })
  it('shows loading state when isLoading', async () => {
    const { SubtaskForm } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskForm')
    render(<SubtaskForm onSubmit={vi.fn()} onCancel={vi.fn()} isLoading={true} />)
    expect(screen.getByTestId('icon-Loader2')).toBeInTheDocument()
  })
  it('submits on Enter key', async () => {
    const onSubmit = vi.fn()
    const { SubtaskForm } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskForm')
    render(<SubtaskForm onSubmit={onSubmit} onCancel={vi.fn()} isLoading={false} />)
  ...
  it('cancels on Escape key', async () => {
    ...
    const { SubtaskForm } = await import('@/components/Dashboard/TaskDetails/SubtasksSection/SubtaskForm')
    render(<SubtaskForm onSubmit={vi.fn()} onCancel={onCancel} isLoading={false} />)
    fireEvent.keyDown(screen.getByPlaceholderText('Subtask title…'), { key: 'Escape' })
    expect(onCancel).toHaveBeenCalled()
  })
