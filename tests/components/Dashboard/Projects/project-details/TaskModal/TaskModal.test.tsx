...
describe('ProjectDetails/TaskModal – TaskTitleInput', () => {
  it('renders title input', async () => {
    const { TaskTitleInput } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskTitleInput')
    render(<TaskTitleInput value="" onChange={vi.fn()} />)
    expect(screen.getByPlaceholderText('What needs to be done?')).toBeInTheDocument()
  })
  it('shows error message', async () => {
    const { TaskTitleInput } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskTitleInput')
    render(<TaskTitleInput value="" onChange={vi.fn()} error="Title is required" />)
...
describe('ProjectDetails/TaskModal – TaskDescriptionInput', () => {
  it('renders textarea', async () => {
    const { TaskDescriptionInput } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskDescriptionInput')
    render(<TaskDescriptionInput value="" onChange={vi.fn()} />)
    expect(screen.getByPlaceholderText('Optional context for this task')).toBeInTheDocument()
  })
})

describe('ProjectDetails/TaskModal – TaskPriorityPicker', () => {
  it('renders all priority options', async () => {
    const { TaskPriorityPicker } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskPriorityPicker')
    render(<TaskPriorityPicker value="MEDIUM" onChange={vi.fn()} />)
  ...
  it('calls onChange on click', async () => {
    const onChange = vi.fn()
    const { TaskPriorityPicker } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskPriorityPicker')
    render(<TaskPriorityPicker value="MEDIUM" onChange={onChange} />)
    fireEvent.click(screen.getByText('HIGH'))
    expect(onChange).toHaveBeenCalledWith('HIGH')
  })
})

describe('ProjectDetails/TaskModal – TaskDueDateInput', () => {
  it('renders date input', async () => {
    const { TaskDueDateInput } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskDueDateInput')
    const { container } = render(<TaskDueDateInput value="" onChange={vi.fn()} />)
    expect(screen.getByText('Due Date')).toBeInTheDocument()
    expect(container.querySelector('input[type="date"]')).toBeInTheDocument()
  })
})

describe('ProjectDetails/TaskModal – TaskStartDateInput', () => {
  it('renders date input', async () => {
    const { TaskStartDateInput } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskStartDateInput')
    const { container } = render(<TaskStartDateInput value="" onChange={vi.fn()} />)
    expect(screen.getByText('Start Date')).toBeInTheDocument()
    expect(container.querySelector('input[type="date"]')).toBeInTheDocument()
  })
})

describe('ProjectDetails/TaskModal – TaskEstimatedHoursInput', () => {
  it('renders number input', async () => {
    const { TaskEstimatedHoursInput } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskEstimatedHoursInput')
    render(<TaskEstimatedHoursInput value={null} onChange={vi.fn()} />)
    expect(screen.getByPlaceholderText('e.g. 2.5')).toBeInTheDocument()
  })
})

describe('ProjectDetails/TaskModal – TaskIntentPicker', () => {
  it('renders intent options', async () => {
    const { TaskIntentPicker } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskIntentPicker')
    render(<TaskIntentPicker value="EXECUTION" onChange={vi.fn()} />)
  ...
  it('calls onChange on click', async () => {
    const onChange = vi.fn()
    const { TaskIntentPicker } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskIntentPicker')
    render(<TaskIntentPicker value="EXECUTION" onChange={onChange} />)
    fireEvent.click(screen.getByText('Think & Plan'))
    expect(onChange).toHaveBeenCalledWith('PLANNING')
  })
})

describe('ProjectDetails/TaskModal – TaskAssigneePicker', () => {
  const members = [
    { id: 'm1', userId: 'u1', user: { id: 'u1', name: 'Alice', email: 'alice@test.com', image: null } },
    { id: 'm2', userId: 'u2', user: { id: 'u2', name: 'Bob', email: 'bob@test.com', image: null } },
  ]
  it('renders members', async () => {
    const { TaskAssigneePicker } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskAssigneePicker')
    render(<TaskAssigneePicker projectMembers={members} selectedUserIds={[]} onToggle={vi.fn()} />)
    expect(screen.getByText('Alice')).toBeInTheDocument()
    expect(screen.getByText('Bob')).toBeInTheDocument()
  })
  it('shows check for selected user', async () => {
    const { TaskAssigneePicker } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskAssigneePicker')
    render(<TaskAssigneePicker projectMembers={members} selectedUserIds={['u1']} onToggle={vi.fn()} />)
    expect(screen.getByText('Alice')).toBeInTheDocument()
  })
  it('calls onToggle on click', async () => {
    const onToggle = vi.fn()
    const { TaskAssigneePicker } = await import('@/components/dashboard/projects/project-details/TaskModal/TaskAssigneePicker')
    render(<TaskAssigneePicker projectMembers={members} selectedUserIds={[]} onToggle={onToggle} />)
