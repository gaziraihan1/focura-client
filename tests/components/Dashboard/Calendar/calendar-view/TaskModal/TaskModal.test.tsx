...
vi.mock('@/components/dashboard/labels/LabelPicker', () => ({
...
describe('CalendarView/TaskModal – TaskModalHeader', () => {
  it('renders title', async () => {
    const { TaskModalHeader } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskModalHeader')
    render(<TaskModalHeader title="My Task" status="TODO" priority="HIGH" isOverdue={false} onClose={vi.fn()} />)
    expect(screen.getByText('My Task')).toBeInTheDocument()
  })
  it('renders status and priority badges', async () => {
    const { TaskModalHeader } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskModalHeader')
    render(<TaskModalHeader title="T" status="COMPLETED" priority="LOW" isOverdue={false} onClose={vi.fn()} />)
    expect(screen.getByText('COMPLETED')).toBeInTheDocument()
    expect(screen.getByText('LOW')).toBeInTheDocument()
  })
  it('shows overdue badge when isOverdue', async () => {
    const { TaskModalHeader } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskModalHeader')
    render(<TaskModalHeader title="T" status="TODO" priority="HIGH" isOverdue={true} onClose={vi.fn()} />)
    expect(screen.getByText('Overdue')).toBeInTheDocument()
  })
  it('hides overdue badge when not overdue', async () => {
    const { TaskModalHeader } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskModalHeader')
    render(<TaskModalHeader title="T" status="TODO" priority="HIGH" isOverdue={false} onClose={vi.fn()} />)
    expect(screen.queryByText('Overdue')).not.toBeInTheDocument()
  })
  it('calls onClose', async () => {
    const onClose = vi.fn()
    const { TaskModalHeader } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskModalHeader')
    render(<TaskModalHeader title="T" status="TODO" priority="HIGH" isOverdue={false} onClose={onClose} />)
    fireEvent.click(screen.getByTestId('icon-X'))
    expect(onClose).toHaveBeenCalled()
  })
})

describe('CalendarView/TaskModal – TaskDescription', () => {
  it('returns null when no description', async () => {
    const { TaskDescription } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskDescription')
    const { container } = render(<TaskDescription description={null} />)
    expect(container.innerHTML).toBe('')
  })
  it('renders description', async () => {
    const { TaskDescription } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskDescription')
    render(<TaskDescription description="This is a description" />)
    expect(screen.getByText('This is a description')).toBeInTheDocument()
  })
})

describe('CalendarView/TaskModal – TaskTimeDetails', () => {
  it('renders created date', async () => {
    const { TaskTimeDetails } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskTimeDetails')
    render(<TaskTimeDetails createdAt="2025-01-01T00:00:00Z" isOverdue={false} />)
    expect(screen.getByText('Created:')).toBeInTheDocument()
  })
  it('renders start date when present', async () => {
    const { TaskTimeDetails } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskTimeDetails')
    render(<TaskTimeDetails startDate="2025-01-01T00:00:00Z" createdAt="2025-01-01T00:00:00Z" isOverdue={false} />)
    expect(screen.getByText('Start Date:')).toBeInTheDocument()
  })
  it('renders due date when present', async () => {
    const { TaskTimeDetails } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskTimeDetails')
    render(<TaskTimeDetails dueDate="2025-01-31T00:00:00Z" createdAt="2025-01-01T00:00:00Z" isOverdue={false} />)
    expect(screen.getByText('Due Date:')).toBeInTheDocument()
  })
  it('renders estimated hours when present', async () => {
    const { TaskTimeDetails } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskTimeDetails')
    render(<TaskTimeDetails estimatedHours={10} createdAt="2025-01-01T00:00:00Z" isOverdue={false} />)
    expect(screen.getByText('Estimated Time:')).toBeInTheDocument()
    expect(screen.getByText('10h')).toBeInTheDocument()
  })
})

describe('CalendarView/TaskModal – TaskPeopleSection', () => {
  it('renders creator name', async () => {
    const { TaskPeopleSection } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskPeopleSection')
    render(<TaskPeopleSection createdBy={{ id: 'u1', name: 'Alice' }} assignees={[]} />)
    expect(screen.getByText('Alice')).toBeInTheDocument()
  })
  it('renders personal task when no assignees', async () => {
    const { TaskPeopleSection } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskPeopleSection')
    render(<TaskPeopleSection createdBy={{ id: 'u1', name: 'Alice' }} assignees={[]} />)
...
describe('CalendarView/TaskModal – TaskProjectSection', () => {
  it('returns null when no project', async () => {
    const { TaskProjectSection } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskProjectSection')
    const { container } = render(<TaskProjectSection project={null} />)
    expect(container.innerHTML).toBe('')
  })
  it('renders project', async () => {
    const { TaskProjectSection } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskProjectSection')
    render(<TaskProjectSection project={{ id: 'p1', name: 'My Project', color: '#3b82f6', workspace: { id: 'w1', name: 'Workspace' } }} />)
    expect(screen.getByText('My Project')).toBeInTheDocument()
    expect(screen.getByText('Workspace')).toBeInTheDocument()
  })
})

describe('CalendarView/TaskModal – TaskActivityStats', () => {
  it('renders counts', async () => {
    const { TaskActivityStats } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskActivityStats')
    render(<TaskActivityStats commentsCount={5} subtasksCount={3} filesCount={2} />)
  ...
  it('renders labels', async () => {
    const { TaskActivityStats } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskActivityStats')
    render(<TaskActivityStats commentsCount={0} subtasksCount={0} filesCount={0} />)
...
describe('CalendarView/TaskModal – TaskModalContent', () => {
  it('renders all sub-sections', async () => {
    const { TaskModalContent } = await import('@/components/dashboard/calendar/calendar-view/TaskModal/TaskModalContent')
    render(
      <TaskModalContent
        description="Desc" startDate="2025-01-01" dueDate="2025-01-31"
        estimatedHours={8} createdAt="2025-01-01" isOverdue={false}
        createdBy={{ id: 'u1', name: 'Alice' }} assignees={[]}
        project={null} commentsCount={1} subtasksCount={2} filesCount={3}
      />
    )
...
describe('CalendarView/TaskModal – UserAvatar', () => {
  it('renders initials when no image', async () => {
    const UserAvatar = (await import('@/components/dashboard/calendar/calendar-view/TaskModal/UserAvatar')).default
    render(<UserAvatar name="Alice Smith" />)
    expect(screen.getByText('AS')).toBeInTheDocument()
  })
  it('renders image when provided', async () => {
    const UserAvatar = (await import('@/components/dashboard/calendar/calendar-view/TaskModal/UserAvatar')).default
    render(<UserAvatar name="Alice" image="/avatar.png" />)
    expect(screen.getByRole('img')).toBeInTheDocument()
  })
  it('applies sm size class', async () => {
    const UserAvatar = (await import('@/components/dashboard/calendar/calendar-view/TaskModal/UserAvatar')).default
    render(<UserAvatar name="Alice" size="sm" />)
    expect(screen.getByText('A')).toBeInTheDocument()
  })
  it('applies lg size class', async () => {
    const UserAvatar = (await import('@/components/dashboard/calendar/calendar-view/TaskModal/UserAvatar')).default
    render(<UserAvatar name="Alice" size="lg" />)
    expect(screen.getByText('A')).toBeInTheDocument()
  })
})
