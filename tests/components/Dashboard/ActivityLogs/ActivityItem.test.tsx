...
describe('ActivityLogs/ActivityItem', () => {
  const baseActivity = {
  ...
  it('renders user name', async () => {
    const { ActivityItem } = await import('@/components/Dashboard/ActivityLogs/ActivityItem')
  ...
  it('renders with compact mode', async () => {
    const { ActivityItem } = await import('@/components/Dashboard/ActivityLogs/ActivityItem')
    render(<ActivityItem activity={baseActivity} compact={true} />)
    expect(screen.getByText('Alice')).toBeInTheDocument()
  })
  it('renders UPDATED action description', async () => {
    const { ActivityItem } = await import('@/components/Dashboard/ActivityLogs/ActivityItem')
    render(<ActivityItem activity={{ ...baseActivity, action: 'UPDATED', metadata: { changes: { title: 'old', description: 'new' } } }} />)
    expect(screen.getAllByText(/Updated:/).length).toBeGreaterThanOrEqual(1)
  })
  it('renders workspace entity type', async () => {
    const { ActivityItem } = await import('@/components/Dashboard/ActivityLogs/ActivityItem')
    render(<ActivityItem activity={{ ...baseActivity, entityType: 'WORKSPACE', task: undefined }} />)
    expect(screen.getByText(/created workspace/)).toBeInTheDocument()
  })
  it('renders project entity type', async () => {
    const { ActivityItem } = await import('@/components/Dashboard/ActivityLogs/ActivityItem')
    render(<ActivityItem activity={{ ...baseActivity, entityType: 'PROJECT', task: undefined, metadata: { projectName: 'Proj' } }} />)
    expect(screen.getByText(/created project/)).toBeInTheDocument()
  })
  it('renders link overlay for task', async () => {
    const { ActivityItem } = await import('@/components/Dashboard/ActivityLogs/ActivityItem')
    const { container } = render(<ActivityItem activity={baseActivity} />)
    const link = container.querySelector('a[href="/dashboard/tasks/t1"]')
