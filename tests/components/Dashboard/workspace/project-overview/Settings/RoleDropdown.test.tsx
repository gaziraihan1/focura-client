...
describe('Workspaces/project/Settings – RoleDropdown', () => {
  it('renders current role badge', async () => {
    const { RoleDropdown } = await import('@/components/dashboard/workspace/project-overview/Settings/RoleDropdown')
    render(
      <RoleDropdown current="MANAGER" memberId="m1" canManage={true} isSelf={false}
        onChangeRole={vi.fn()} onRemove={vi.fn()} />
    )
  ...
  it('does not open when cannot manage', async () => {
    const { RoleDropdown } = await import('@/components/dashboard/workspace/project-overview/Settings/RoleDropdown')
    render(
      <RoleDropdown current="VIEWER" memberId="m1" canManage={false} isSelf={true}
        onChangeRole={vi.fn()} onRemove={vi.fn()} />
    )
    fireEvent.click(screen.getByText('VIEWER').closest('button')!)
    expect(screen.queryByText('Leave project')).not.toBeInTheDocument()
  })
  it('calls onChangeRole', async () => {
    const onChangeRole = vi.fn()
    const { RoleDropdown } = await import('@/components/dashboard/workspace/project-overview/Settings/RoleDropdown')
    render(
      <RoleDropdown current="MANAGER" memberId="m1" canManage={true} isSelf={false}
        onChangeRole={onChangeRole} onRemove={vi.fn()} />
    )
    fireEvent.click(screen.getByText('MANAGER').closest('button')!)
    fireEvent.click(screen.getByText('COLLABORATOR'))
    expect(onChangeRole).toHaveBeenCalledWith('m1', 'COLLABORATOR')
  })
  it('calls onRemove', async () => {
    ...
    render(
      <RoleDropdown current="MANAGER" memberId="m1" canManage={true} isSelf={false}
        onChangeRole={vi.fn()} onRemove={onRemove} />
    )
    fireEvent.click(screen.getByText('MANAGER').closest('button')!)
    fireEvent.click(screen.getByText('Remove member'))
    expect(onRemove).toHaveBeenCalledWith('m1')
  })
  it('shows Leave project for self', async () => {
    const { RoleDropdown } = await import('@/components/dashboard/workspace/project-overview/Settings/RoleDropdown')
    render(
      <RoleDropdown current="MANAGER" memberId="m1" canManage={true} isSelf={true}
        onChangeRole={vi.fn()} onRemove={vi.fn()} />
    )
    fireEvent.click(screen.getByText('MANAGER').closest('button')!)
