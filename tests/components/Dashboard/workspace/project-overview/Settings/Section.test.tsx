...
describe('Workspaces/project/Settings – Section', () => {
  it('renders title', async () => {
    const { Section } = await import('@/components/dashboard/workspace/project-overview/Settings/Section')
    render(<Section title="General">Child content</Section>)
    expect(screen.getByText('General')).toBeInTheDocument()
    expect(screen.getByText('Child content')).toBeInTheDocument()
  })
  it('renders description', async () => {
    const { Section } = await import('@/components/dashboard/workspace/project-overview/Settings/Section')
    render(<Section title="General" description="Manage settings">Content</Section>)
    expect(screen.getByText('Manage settings')).toBeInTheDocument()
  })
  it('applies danger styles', async () => {
    const { Section } = await import('@/components/dashboard/workspace/project-overview/Settings/Section')
    const { container } = render(<Section title="Danger" danger>Content</Section>)
