...
describe('Workspaces/project/Tasks – StatsBar', () => {
  it('renders completion percentage', async () => {
    const { StatsBar } = await import('@/components/dashboard/workspace/project-overview/Tasks/StatsBar')
    render(<StatsBar tasks={[
      { status: 'COMPLETED', id: '1' } as any,
      { status: 'TODO', id: '2' } as any,
      { status: 'IN_PROGRESS', id: '3' } as any,
    ]} />)
    expect(screen.getByText('33% complete')).toBeInTheDocument()
  })
  it('shows loading state', async () => {
    const { StatsBar } = await import('@/components/dashboard/workspace/project-overview/Tasks/StatsBar')
    const { container } = render(<StatsBar tasks={[]} isLoading={true} />)
