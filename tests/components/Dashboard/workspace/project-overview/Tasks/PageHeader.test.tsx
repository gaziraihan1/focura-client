...
describe('Workspaces/project/Tasks – PageHeader', () => {
  it('renders project name', async () => {
    const { PageHeader } = await import('@/components/dashboard/workspace/project-overview/Tasks/PageHeader')
    render(<PageHeader projectName="My Project" projectColor="#3b82f6" totalCount={5} onAddTask={vi.fn()} canCreate={true} isArchived={false} />)
    expect(screen.getByText('My Project')).toBeInTheDocument()
  })
  it('renders task count', async () => {
    const { PageHeader } = await import('@/components/dashboard/workspace/project-overview/Tasks/PageHeader')
    render(<PageHeader projectName="P" projectColor="#3b82f6" totalCount={5} onAddTask={vi.fn()} canCreate={true} isArchived={false} />)
    expect(screen.getByText('5 tasks')).toBeInTheDocument()
  })
  it('renders singular task count', async () => {
    const { PageHeader } = await import('@/components/dashboard/workspace/project-overview/Tasks/PageHeader')
    render(<PageHeader projectName="P" projectColor="#3b82f6" totalCount={1} onAddTask={vi.fn()} canCreate={true} isArchived={false} />)
    expect(screen.getByText('1 task')).toBeInTheDocument()
  })
  it('shows New Task button when canCreate', async () => {
    const { PageHeader } = await import('@/components/dashboard/workspace/project-overview/Tasks/PageHeader')
    render(<PageHeader projectName="P" projectColor="#3b82f6" totalCount={0} onAddTask={vi.fn()} canCreate={true} isArchived={false} />)
    expect(screen.getByText('New Task')).toBeInTheDocument()
  })
  it('hides New Task when cannot create', async () => {
    const { PageHeader } = await import('@/components/dashboard/workspace/project-overview/Tasks/PageHeader')
    render(<PageHeader projectName="P" projectColor="#3b82f6" totalCount={0} onAddTask={vi.fn()} canCreate={false} isArchived={false} />)
    expect(screen.queryByText('New Task')).not.toBeInTheDocument()
  })
  it('hides New Task when archived', async () => {
    const { PageHeader } = await import('@/components/dashboard/workspace/project-overview/Tasks/PageHeader')
    render(<PageHeader projectName="P" projectColor="#3b82f6" totalCount={0} onAddTask={vi.fn()} canCreate={true} isArchived={true} />)
    expect(screen.queryByText('New Task')).not.toBeInTheDocument()
  })
  it('renders workspace name', async () => {
    const { PageHeader } = await import('@/components/dashboard/workspace/project-overview/Tasks/PageHeader')
    render(<PageHeader projectName="P" projectColor="#3b82f6" workspaceName="My Workspace" totalCount={0} onAddTask={vi.fn()} canCreate={true} isArchived={false} />)
