...
describe("EmptyAnnouncements", () => {
  it("renders no announcements yet", async () => {
    const { EmptyAnnouncements } = await import("@/components/dashboard/workspace/project-overview/Announcements/EmptyAnnouncements");
    render(
      <EmptyAnnouncements filtered={false} canManage onNew={vi.fn()} />
    );
    expect(screen.getByText("No announcements yet")).toBeInTheDocument();
    expect(screen.getByText("Create Announcement")).toBeInTheDocument();
  });

  it("renders no matching when filtered", async () => {
    const { EmptyAnnouncements } = await import("@/components/dashboard/workspace/project-overview/Announcements/EmptyAnnouncements");
    render(
      <EmptyAnnouncements filtered canManage={false} onNew={vi.fn()} />
    );
