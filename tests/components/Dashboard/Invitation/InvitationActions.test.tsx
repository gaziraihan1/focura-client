...
describe("InvitationActions", () => {
  it("renders accept and decline buttons", async () => {
    const { default: InvitationActions } = await import("@/components/Dashboard/Invitation/InvitationCard/InvitationActions");
    render(
      <InvitationActions onAccept={vi.fn()} onDecline={vi.fn()} isAccepting={false} />
    );
    expect(screen.getByText("Accept Invitation")).toBeInTheDocument();
    expect(screen.getByText("Decline")).toBeInTheDocument();
  });

  it("shows loading state when accepting", async () => {
    const { default: InvitationActions } = await import("@/components/Dashboard/Invitation/InvitationCard/InvitationActions");
    render(
      <InvitationActions onAccept={vi.fn()} onDecline={vi.fn()} isAccepting />
    );
