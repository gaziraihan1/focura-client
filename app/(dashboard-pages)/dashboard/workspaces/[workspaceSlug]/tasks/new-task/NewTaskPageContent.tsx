...
export function NewTaskPageContent({ workspaceSlug }: NewTaskPageContentProps) {
  const controller = useWorkspaceNewTaskPage({ workspaceSlug });
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <form onSubmit={controller.handleSubmit} className="space-y-6">
        <BasicInformationSection
          ...
          onTitleChange={(title) => controller.setFormData((p) => ({ ...p, title }))}
          onDescriptionChange={(description) => controller.setFormData((p) => ({ ...p, description }))}
          onProjectChange={(projectId) => controller.setFormData((p) => ({ ...p, projectId }))}
        />

        <TaskDetailsSection
          {...controller.formData}
          errors={controller.errors}
          onStatusChange={(status) => controller.setFormData((p) => ({ ...p, status }))}
          onPriorityChange={(priority) => controller.setFormData((p) => ({ ...p, priority }))}
          onIntentChange={(intent) => controller.setFormData((p) => ({ ...p, intent }))}
          onEnergyTypeChange={(energyType) => controller.setFormData((p) => ({ ...p, energyType }))}
          onFocusRequiredChange={(focusRequired) => controller.setFormData((p) => ({ ...p, focusRequired }))}
          onFocusLevelChange={(focusLevel) => controller.setFormData((p) => ({ ...p, focusLevel }))}
          onDistractionCostChange={(distractionCost) => controller.setFormData((p) => ({ ...p, distractionCost }))}
          onStartDateChange={(startDate) => controller.setFormData((p) => ({ ...p, startDate }))}
          onDueDateChange={(dueDate) => controller.setFormData((p) => ({ ...p, dueDate }))}
          onEstimatedHoursChange={(estimatedHours) => controller.setFormData((p) => ({ ...p, estimatedHours }))}
        />
        ...
        <TeamLabelsSection
          ...
          onLabelChange={(labelIds) => controller.setFormData((p) => ({ ...p, labelIds }))}
          onOpenLabelManager={() => controller.setShowLabelManager(true)}
        />
      ...
      {controller.showLabelManager && (
        <LabelManagerModal
          workspaceId={controller.workspace.id}
          onClose={() => controller.setShowLabelManager(false)}
        />
      )}
    </div>
  );
