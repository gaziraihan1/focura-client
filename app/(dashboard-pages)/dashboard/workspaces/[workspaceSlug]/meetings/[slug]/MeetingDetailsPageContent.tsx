...
import { Calendar, Clock, MapPin, Link2, Users, ArrowLeft, Video } from "lucide-react";
...
export function MeetingDetailsPageContent({ workspaceSlug, meetingSlug }: MeetingDetailsPageContentProps) {
  const router = useRouter();

  const { data: workspace } = useWorkspace(workspaceSlug);
  const { data: meeting, isLoading } = useMeeting(workspace?.id ?? "", meetingSlug);
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
        ...
        <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
          <DetailCard className="md:col-span-2 space-y-5">
            ...
            <InfoRow icon={Calendar} label="Date">{formatDate(meeting.startTime)}</InfoRow>
            ...
            {meeting.location && <InfoRow icon={MapPin} label="Location">{meeting.location}</InfoRow>}
            {meeting.link && (
              <InfoRow icon={Link2} label="Meeting Link">
                <a href={meeting.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg bg-primary/10 px-3 py-1.5 text-sm font-medium text-primary transition-colors hover:bg-primary/20">
        ...
        <DetailCard>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Attendees</h2>
            <span className="rounded-full bg-muted px-2.5 py-0.5 text-xs font-medium text-muted-foreground">{attendeeCount}</span>
          </div>
          {attendeeCount === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              {meeting.attendees.map((attendee) => (
                <AttendeeAvatar key={attendee.id} user={attendee.user} joinedAt={attendee.joinedAt} />
              ))}
            </div>
          )}
        </DetailCard>
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          Created{" "}
          {new Date(meeting.createdAt).toLocaleDateString("en-US", { timeZone: "UTC", month: "short", day: "numeric", year: "numeric" })}
          {meeting.updatedAt !== meeting.createdAt && (
            <> · Updated{" "}
              {new Date(meeting.updatedAt).toLocaleDateString("en-US", { timeZone: "UTC", month: "short", day: "numeric", year: "numeric" })}
            </>
          )}
        </p>
      </div>
