...
export function ProjectSettingsPageContent({ workspaceSlug, projectSlug }: ProjectSettingsPageContentProps) {
  return (
    <Suspense fallback={null}>
      <ProjectSettingsContent workspaceSlug={workspaceSlug} projectSlug={projectSlug} />
    </Suspense>
  );
}

function ProjectSettingsContent({ workspaceSlug, projectSlug }: ProjectSettingsPageContentProps) {
  ...
  const { data: project, isLoading, isRefetching } = useProjectDetailsBySlug(projectSlug);
  ...
  const isResolved = !isLoading && !userLoading && !workspaceRole.isLoading && !!project && !!userId;
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        {activeTab === "general" && <GeneralTab project={project} canManage={canManage} />}
        {activeTab === "members" && <MembersTab project={project} canManage={canManage} userId={userId} />}
        {activeTab === "appearance" && <AppearanceTab project={project} canManage={canManage} />}
        {activeTab === "danger" && <DangerTab project={project} canManage={canManage} isOwnerOrAdmin={isOwnerOrAdmin} workspaceSlug={workspaceSlug} />}
      </div>
    </div>
  );
}
