...
import { Wrench, Github, MessageSquare, Calendar, Trello, Loader2, Check, ExternalLink } from 'lucide-react';
...
const WORKSPACE_INTEGRATIONS = [
  ...
  },
  {
    id: 'slack',
    name: 'Slack',
    description: 'Post task updates and receive notifications in channels',
    icon: MessageSquare,
    color: 'bg-[#4A154B]/10',
...
export function WorkspaceIntegrationsForm({ workspaceSlug }: WorkspaceIntegrationsFormProps) {
  ...
  useEffect(() => {
    const fetchIntegrations = async () => {
      try {
        const result = await api.get<Integration[]>(`/api/v1/workspace-integrations/${workspaceSlug}`, { showErrorToast: false });
  ...
  const handleConnect = async (provider: string) => {
    ...
    try {
      await api.post(`/api/v1/workspace-integrations/${workspaceSlug}`, { provider });
      toast.success(`${provider} connected`);
      // Re-fetch
      const result = await api.get<Integration[]>(`/api/v1/workspace-integrations/${workspaceSlug}`, { showErrorToast: false });
      if (result?.success && result.data) setIntegrations(result.data);
    } catch {
  ...
  const handleDisconnect = async (integrationId: string, name: string) => {
    ...
    try {
      await api.delete(`/api/v1/workspace-integrations/${workspaceSlug}/${integrationId}`);
    ...
    <div className="space-y-8">
      <div className="rounded-2xl border border-border bg-card p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-sm font-semibold tracking-tight">Workspace Integrations</h3>
            <p className="text-xs text-muted-foreground mt-0.5">
              Connect tools for your entire team
            </p>
          </div>
        </div>
