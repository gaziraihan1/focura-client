...
export function ProjectTimeReport({
  ...
  projectId,
  projectName,
}: ProjectTimeReportProps) {
  const [days, setDays] = useState(7);
  const { data, isLoading, isError, refetch } = useProjectTimeReport(
    workspaceId,
    projectId,
    days
  );
                ...
                {data.entries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-accent/30 transition-colors">
                    ...
                    <td className="px-3 py-2.5 min-w-40">
                      <p className="truncate max-w-56" title={entry.taskTitle ?? ""}>
