...
export function EndpointHeader({ endpoint, open, onToggle }: EndpointHeaderProps) {
    ...
    >
      <div className='flex items-start gap-3 min-w-0 flex-1'>
        ...
        <div className='min-w-0'>
          <code className='text-sm font-mono text-foreground break-all leading-snug'>
            {endpoint.path}
          </code>
      ...
      <div className='flex gap-0 border-y border-border bg-muted/40 overflow-x-auto'>
        {availableTabs.map((t) => (
          ...
          >
            {TAB_LABELS[t]}
            {t === 'params' && hasParams && (
              <span className='ml-1.5 text-[10px] bg-muted text-muted-foreground rounded-full px-1.5 py-0.5 font-bold'>
...
// ─── Params table ─────────────────────────────────────────────────────────────
export function ParamsTable({ params, title }: {
          ...
          <thead>
            <tr className='bg-muted/60 border-b border-border'>
              {['Name', 'Type', 'Required', 'Description'].map((h) => (
                <th key={h} className='text-left px-3 py-2 text-muted-foreground font-semibold whitespace-nowrap'>
                  {h}
                </th>
              ))}
            </tr>
...
// ─── Response list ────────────────────────────────────────────────────────────
export function ResponseList({ responses }: { responses: Endpoint['responses'] }) {
  return (
    <div className='space-y-3'>
      {responses.map(({ status, description, shape }) => (
        <div key={status} className='rounded-xl border border-border overflow-hidden'>
          <div className='flex items-center gap-3 px-3 py-2.5 bg-muted/60'>
            <span className={cn(
            ...
            <table className='w-full text-xs border-collapse border-t border-border'>
              <tbody>
                {shape.map((field, i) => (
                  <tr
                    key={field.name}
                    className={i < shape.length - 1 ? 'border-b border-border' : ''}
                  >
                    ...
                    <td className='px-3 py-2 align-top w-1/6'>
                      <code className='font-mono text-muted-foreground'>{field.type}</code>
                    </td>
...
// ─── Code examples ────────────────────────────────────────────────────────────
export function CodeExamples({ examples }: { examples: Endpoint['examples'] }) {
  ...
  return (
    <div className='rounded-xl border border-border overflow-hidden'>
      ...
      {/* Code block */}
      {current && (
        <pre className='overflow-x-auto p-4 text-[12px] font-mono text-foreground bg-muted/60 leading-relaxed'>
