...
export const frontendArchArticles: GuideArticle[] = [
  {
    id: "layered-architecture",
    title: "Layered architecture",
    summary:
      "Logic lives in hooks, UI components are display-only. The layer order is types → lib/axios → hooks → components → app, with no business logic in pages.",
    content: (
      <>
        <Prose>
          The client follows a strict layered architecture. Logic lives in hooks — UI components are
          display-only. No business logic in pages or components.
        </Prose>
...
└── utils/                # Pure utility functions`}</FileTree>
  ...
  },
  {
    ...
    summary:
      "Every hook uses a queryKeys factory for precise cache invalidation, plus useQuery/useMutation examples for task lists.",
    content: (
      <>
        <SectionH>React Query patterns</SectionH>
        <Prose>
          Every hook uses a <IC>queryKeys</IC> factory so cache invalidation is always precise.
        </Prose>
...
export function useCreateTask(workspaceId: string) {
  ...
  },
  {
    ...
    title: "Key rules",
    summary:
      "Always name the QueryClient instance qc, keep mutationFn inline, guard queries with enabled, and keep all logic out of UI components.",
    content: (
      <>
        ...
        <Tip>
          When a hook needs <IC>useRouter</IC> or other client-only APIs, extract it into its own{" "}
          <IC>&quot;use client&quot;</IC> file. Mixing it into a server component import trace causes
          build errors.
        </Tip>
      </>
    ),
